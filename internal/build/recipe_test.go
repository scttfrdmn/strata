...
func TestParseRecipeValid(t *testing.T) {
	...
	if r.Meta.Family != "rhel" {
		t.Errorf("Family = %q, want %q", r.Meta.Family, "rhel")
	}
...
func TestToLayerManifest(t *testing.T) {
	...
	if manifest.Family != "rhel" {
		t.Errorf("Family = %q, want %q", manifest.Family, "rhel")
	}
	...
	// ID should be name-version-family-arch.
	wantID := "openmpi-4.1.6-rhel-x86_64"
