...
| T2, T7, T9 | `strata verify` and `IsSigned()` are presence checks; no lockfile verification exists | A1 | #60 | Partially — **2026-09-14 (v0.26)**. Lockfile verification now exists and is real: `trust.VerifyLockFile` (`internal/trust/lockfile.go`) checks a cosign signature over the whole layer set, and `strata verify` calls it against the public key embedded in the binary (`internal/trust/anchor.go`), so a mix-and-match of individually-valid layers is caught (demonstrated end-to-end: `strata sign` against the KMS key logs to Rekor, `strata verify` confirms with no AWS, and tampering a layer digest fails the check). `trust.SignLockFile` + `strata sign` produce the signature. **Re-derived** at E1 (chosen/implementation): `internal/trust/lockfile_test.go` (round-trip + tamper/mix-and-match/unsigned), `cmd/strata/sign_test.go`. Still live, so `Partially`: `IsSigned()` remains a one-line presence predicate, `strata publish` does not yet call `VerifyLockFile`, the EC2 agent does not yet verify the lockfile signature before mounting, and an *unsigned* lockfile still passes `verify`'s default path (verification is not yet mandatory) — those close as the wiring completes (#101 for the set/freshness half) |
