...
**Status: first population, 2026-08-21.** Every proposition below carries a
verdict from an adversarial review of the proposition *text*, and a status with
cited evidence or an explicit `UNPOPULATED` and the reason it could not be
settled. The bibliography in §5 has been verified against primary sources.
...
| working tree | clean at that commit; this file is the only addition |
...
distinct paths by which a layer could be mounted without verification have been
found by reading code. Each was real. None of them told us how many remain. A
stated property with a model behind it can be *checked*; a property that lives
only in prose can only be spot-checked forever.
...
**Two of the five fail-open paths found in this codebase need no adversary at
...
- **Verdict** — the result of attacking the proposition's *text*: `SOUND`,
  `TOO WEAK` (satisfiable by an implementation that still has the defect the
  proposition was written to exclude — the broken implementation is named),
  `TOO STRONG`, or `ILL-FORMED` (with the falsifiable replacement).
- **Status** — the result of measuring the *implementation*: `ENFORCED` with a
  tier, `ASSERTED` (E0), `REFUTED` with the counterexample, `WITHDRAWN`, or
  `UNPOPULATED` with the reason.
...
Four propositions — R2, R3, R6, P1 — were `TOO WEAK` for the *same structural
reason*: each defers its content to a definition the implementation controls
(the spec's exception clause, the spec's field enumeration, `IsFrozen()`). Each
can be satisfied by weakening that definition rather than by fixing anything. A
proposition whose subject is "whatever the specification says" is not a
constraint on the specification.

T1 is the fifth and the most consequential: it defers to "the trust policy in
force" and never fixes the default. That is the fail-open shape restated as a
property, which is why T1 alone could have been claimed satisfied throughout the
period in which five fail-open paths existed.
...
describing the code rather than constraining it.
...
| I6, P3, X1 | `packages:` installs ignore the recorded SHA256; conda `latest` resolves at boot | A3, A7, H1 | #98 | No |
...
remaining 23 are `SOUND`. Of the 34 propositions, 29 carry a refutation, 7 carry
an E1 enforcement, 4 carry both (enforced on one route, refuted on another), 1 is
`ASSERTED` at E0 and 1 is `UNPOPULATED` with its reason. Re-derive:

```
awk -F'|' '/^\| \*\*[RITPBX][0-9]|^\| \*\*I1′/{tot++;
  if ($5 ~ /REFUTED/) r++; if ($5 ~ /ENFORCED/) e++;
  if ($5 ~ /REFUTED/ && $5 ~ /ENFORCED/) b++;
  if ($4 ~ /SOUND/) s++}
  END{print tot, s, r, e, b}' PROPERTIES.md
```

The
propositions are left as written with the verdict beside them, because rewriting
a proposition to match what the code does is the failure §6 exists to prevent.
§3.1 records the structural pattern the four `TOO WEAK` verdicts share.
