...
verified against primary sources. Of the 34 propositions, 14 are `REFUTED`, 12
are `ENFORCED E1`, 1 is `ENFORCED exhaustive/implementation`, 1 is
`ASSERTED (E0)`, 4 are `UNPOPULATED` and 2 are
...
> (`internal/propdoc/propdoc.go:342`) and `Kind` discards the parenthesised detail
...
| live rows current through | the v0.26 signing work — lockfile signing (#60), all-boundary verification, the embedded-key discharge (#62), and the two re-review fixes (#219 dedup ID-collision, #220 rekor-entry binding). `propgen` re-derives the Status column and the distribution header at every commit (CI), so the counts track HEAD; the `measured at` commit above is the last *full manual* re-audit (#104), not the currency of the live rows |
...
| **A7** | **Upstream mutation of an unpinned dependency.** Change what a name resolves to at a third-party package index between resolution and assembly, without touching Strata's registry. | Distinct from A3: no interposition, no compromise. `conda install <name>` where the recorded version is `latest` (`internal/agent/package_installer.go:118`) and `pip install name==version` with no hash (`:101`) both admit it. Added 2026-08-21; see §7. |
...
That nomination is no longer a prediction. `internal/agent/agent.go:288-296`
...
**Closed by #104.** That block no longer exists; `internal/agent/agent.go:313-326`
...
all.** `internal/agent/agent.go:288` skips verification when the caller supplied
...
- **`Reduce` consumes the order on `Coverage` alone** — `BasisKind.Rank`,
  `internal/propdoc/basis.go:105` — which §2 states is total and means it. It never
...
6. **A test that does not execute the property's site is not E1 for that
   ...
   *line* runs. Both have been violated here. Inverting
   `internal/agent/agent.go:288-296` **at `339329fb`** left all seven tests in the
...
7. **A documented claim that is false is not E0 — it is a refutation, and the
   ...
   it the same as an honest silence. `internal/trust/verify.go:93-94` asserts
...
11. **Closure-discharged is not evidence-discharged.** An issue closing is a fact
    ...
    **Half of this rule is now enforced, and the boundary matters more than the
    enforcement.** `propdoc.Doc.DischargeDefects` (`internal/propdoc/propdoc.go:453`)
...
12. **A proposition covered unevenly across its domain reduces to its weakest
    ...
    **A meet needs an order, and the seven bases do not have one.** Coverage is
    ...
    (`internal/propdoc/basis.go:185`, `internal/propdoc/propdoc.go:585`), tested at
    ...
    the other 32 are single-scope and unchanged. When this rule was added both
    scoped propositions were `REFUTED`, and a live refutation outranks any basis
    (rule 5), so neither Status cell displayed its reduction — the generated column
    was byte-identical before and after the rule, and the renderings were covered
    only by constructed cells (§7 item 80). **That changed when T1 and T5 were
    discharged (#172, 2026-09-13):** with no live refutation left, each Status now
    renders its reduction — `ENFORCED E1 (weakest of 2 scopes)` for `T1`,
    `... 3 scopes` for `T5` — so the document itself now witnesses the multi-scope
    rendering. (Added 2026-08-22, #135; updated 2026-09-13, #172.)
...
**Scope: "the assembled environment"** (used by R7, X2, X1). It is the mounted
layer content plus the process environment exported into the assembled root — the
`/etc/profile.d` scripts, `/etc/strata/environment`, and the child process env —
and it **excludes the provenance record**: the lockfile copied verbatim to
`/etc/strata/active.lock.yaml` (`internal/overlay/overlay.go:224-231`) and the
`STRATA_REKOR_ENTRY` export. Without this clause R7's premise is unsatisfiable —
every lockfile field lands byte-for-byte in `active.lock.yaml`, so no two distinct
lockfiles assemble "the same environment" and the R7 search covers an empty domain
(#121).

The exclusion is **per field, not per file.** `active.lock.yaml` is not inert:
`cmd/strata/cache_prune.go:38,59-62,120` reads `Layers[].SHA256` out of it and
deletes every cached `.sqfs` not in that set. That is safe only because the one
field it reads is in `envHashInput`; a blanket "the provenance record is out of
scope" would let a future reader of an *unhashed* field drive that destructive
action while the identity stayed blind to it. So a field written into
`active.lock.yaml` is out of scope exactly while nothing outside the provenance
record reads it — the moment a consumer like `cache_prune` reads one, it re-enters
scope and must participate in the identity.

### Group R — Resolution

| # | Proposition | Verdict | Basis | Status | Evidence |
|---|-------------|---------|-------|--------|----------|
| **R1** | *Determinism.* Resolution is a function of (profile, registry state, resolver version). Two resolutions with identical inputs produce lockfiles that are byte-identical after eliding exactly two wall-clock fields, `resolved_at` (the time of resolution) and `base.capabilities.probed_at` (the time of the base probe) — each a clock reading, and each excluded from the identity by design (`EnvironmentID` and `BaseCapabilities.ContentDigest` respectively, `spec/base_digest.go:15`). Every other byte is identical — including every field that does not participate in `environment_id`. The set of elided fields is enumerated here and is not deferred to the specification. | SOUND | chosen/implementation — `internal/resolver/resolve_determinism_test.go:73` `TestResolve_IdenticalInputsAreByteIdenticalElidingResolvedAt` | ENFORCED E1 | **Restated 2026-08-21** (was: *"...produce byte-identical lockfiles"*, verdict `TOO STRONG`). The old form was unsatisfiable — a lockfile records `ResolvedAt` from the wall clock, so byte-identity fails on input one and satisfying R1 literally meant abandoning a field Strata deliberately records. **What the old statement permitted that this one forbids:** an unsatisfiable proposition generates no checkable obligation, so what stood in for R1 was whatever its citation happened to assert. `internal/resolver/resolver_test.go:601 TestEnvironmentID_Stability` compares `EnvironmentID` and `ProfileSHA256` — two derived hashes, the first over the five members of `envHashInput` (`spec/lockfile_hash.go:19-25`). Every field outside that hash was therefore free to differ between two identical resolutions with nothing objecting: `profile_name`, `strata_version`, `rekor_entry`, `bundle`, `mutable_layer`, `mount_order`, `satisfied_by`, `from_formation`. The new form forbids all of that. **What can now refute it that could not before:** a differential resolve comparing the full canonical serialisation with `resolved_at` elided. Under the old form no such test could be R1 evidence, because an honest attempt fails on the timestamp; under the new form it is the direct instrument, and it is cheap. Basis is deliberately `none`: the cited test does not exercise the restated property (proof-standard rule 6), and the old cell's `ENFORCED E1` measured a *repaired form* — "identical on the canonical content projection" — that deferred the word *canonical* to the implementation, which is rule 9. Stating R1 truthfully moved it from `ENFORCED E1` to `UNPOPULATED`. **Update 2026-09-14 (#95): the instrument now exists.** `internal/resolver/resolve_determinism_test.go:73` resolves the same profile twice and compares the full YAML serialisation with the wall-clock fields elided; a difference in any other field reddens it, and a control asserts the two serialisations differ *before* eliding, so the elision cannot pass vacuously. Writing it found the enumeration incomplete: R1 named only `resolved_at`, but `base.capabilities.probed_at` is a second clock reading, excluded from the base content digest (`spec/base_digest.go:15`) exactly as `resolved_at` is excluded from `environment_id`, so it too must be elided — now enumerated above. Basis moves `none` → `chosen/implementation` (E1); the fixture is three independent layers, chosen not exhaustive, so the claim says nothing about profiles with formations or dependency edges. |
| **R2** | *Order-independence.* Permuting the entries of `software:` in a profile does not change the resulting lockfile in any field that describes the assembled environment — `mount_order`, all layer content, and `environment_id` included. It may change exactly the fields that record the input or the clock rather than the environment: `profile_sha256`, which hashes the profile bytes and so moves when `software:` is reordered (the same category as R1's `resolved_at`), and the two wall-clock fields `resolved_at` and `base.capabilities.probed_at`. This elided set is enumerated here, not deferred to the specification, and no spec amendment can widen it. Where the dependency graph leaves two layers mutually unordered, their relative order is fixed by a total order computed from layer content — name, then version, then digest — and never by position in the input. | SOUND | chosen/implementation — `internal/resolver/resolve_determinism_test.go:125` `TestResolve_PermutingSoftwareIsByteIdenticalElidingInputRecordingFields` | ENFORCED E1 | **Restated 2026-08-21** (was: *"...does not change the resulting lockfile, except in fields whose specification says they record declaration order"*, verdict `TOO WEAK`). **What the old statement permitted that this one forbids:** one sentence added to the spec — *"`mount_order` records declaration order"* — satisfied the old R2 exactly, while leaving YAML key order to decide OverlayFS shadowing between two layers that both ship `bin/python`. The old exception clause was also open-ended: any field could be exempted later by amending the spec, so the proposition's content was whatever the implementation's own document said it was (rule 9). The new form forbids both — `mount_order` must be permutation-invariant, the tie-break is named in the proposition rather than referenced, and no spec amendment can widen it. **What can now refute it that could not before:** a permutation generator over `software:`. Under the old form that generator **confirms** R2 no matter what the implementation does, because every difference it can find lands in `mount_order`, which the spec exempts — a property test of a too-weak proposition is a tautology. Under the new form the same generator refutes on input one, and two artifacts already in hand become refutations of R2 itself rather than of a doc comment. Counterexample, executed: two lockfiles differing only in the slice order of two layers that share `MountOrder` yield different `EnvironmentID`s (`ef104908…` vs `4535c5c4…`), with the distinct-`MountOrder` control returning equal (`ef104908…` twice). The doc comment at `spec/lockfile_hash.go:30-31` claims determinism "regardless of the order they appear in the lockfile YAML" — true only when `MountOrder` is distinct. Upstream, the resolver's tie-break *was* declaration order: `internal/resolver/stages.go:304` `sort.Ints(queue) // deterministic tie-breaking` ordered by input index. Filed as #95. **Update 2026-09-13 (the `sort.Ints` line above is now the pre-fix record):** `stage6TopoSort` tie-breaks the ready set by `lessByLayerContent` (name, version, ID), so permuting `software:` no longer changes `MountOrder` or `EnvironmentID` — `internal/resolver/mountorder_permutation_test.go`, red under the old code. R2 stays REFUTED because it forbids changing **any** field and `ProfileSHA256` still records the input order (the same shape as R1's `resolved_at`); the `Packages`-permutation clause is not an R2 matter and is content rather than a defect. **Restated and discharged 2026-09-14 (#95).** The `mount_order`/`environment_id` invariance now has a full-serialisation instrument, not just the by-name mount-order check: `internal/resolver/resolve_determinism_test.go:125` marshals the whole lockfile forward and reversed and compares with the enumerated fields elided — red under the old `sort.Ints(queue)` (mutation-checked: reordering by input index reddens it; reversing the *content* order does not, because a reversed total order is still permutation-invariant), green now. The two things that kept the row live are resolved rather than pending: (a) the `MountOrder`-tie half in hand-built lockfiles is closed at the trust boundary by `LockFile.Validate()` (#65/#96, `spec/lockfile_validate_test.go`), which rejects a duplicate `mount_order` before any run/publish/freeze acts on it; (b) the `ProfileSHA256` "any field" refutation was R2's own too-strong wording, not a code defect — the narrowing above elides it as an input-recording field, exactly as R1 elides `resolved_at`, and writing the instrument added `base.capabilities.probed_at` to that enumerated set. R2's Basis moves `none` → `chosen/implementation` (E1) and the #95 refutation is discharged. **It does not reach `ENFORCED`, though:** the post-release adversarial review found a second counterexample the standalone fixture could not — two formations sharing a layer mount it twice and attach `satisfied_by`/`from_formation` to the duplicates in `software:` order (#208), which R2 does not elide. So R2 was `SOUND` and evidenced over the standalone-independent domain (`resolve_determinism_test.go`) but `REFUTED` over overlapping formations. **Discharged 2026-09-14 (#208, v0.26):** `dedupLayers` collapses a layer requested through several formations into one instance with sorted-union provenance, so `overlapping_formations_test.go` now confirms permutation-invariance over that domain too. With both #95 and #208 discharged and no live refutation, R2 derives `ENFORCED E1` over standalone layers and overlapping formations. |
| **R3** | *Totality.* Resolution yields either a complete lockfile or an error. No execution produces a partially-populated lockfile. | **TOO WEAK** | none | UNPOPULATED | Broken-but-satisfying implementation: drop every request that cannot be resolved and return a fully-populated lockfile for the remainder. Every layer it names is complete, so R3 holds, and the environment is silently not the one that was asked for. "Complete" must be quantified over the *request*: every element of `software:` is either represented in the lockfile or named in an error. Realised instance: #79 — a null entry in a software list is silently dropped and the profile resolves without it. Second instance, untracked: `Profile.Instance` and `Profile.Storage` are parsed and then referenced nowhere (see X1). |
| **R4** | *Provider soundness.* If the dependency graph contains an edge from a consumer to a provider for capability *c*, that provider satisfies the consumer's declared version constraint on *c*. | SOUND | chosen/implementation — `internal/resolver/provider_matrix_test.go:302 TestStage6ProviderSoundness` | ENFORCED E1 | **Discharged 2026-09-12 by #153** (merged `9e918de`): stage 6 now appends every provider (`internal/resolver/stages.go:322-325`) and `selectProvider` (`:275`) chooses the highest satisfying version, tie-break lowest layer ID, so the wrong edge cannot form — the 3-of-12 violations are 0 and `TestStage6ProviderSoundness` is now a regression barrier where a nonzero count means the defect returned. `TestStage6SelectsHighestSatisfyingProvider` additionally pins version-over-slice-order, which the soundness table cannot distinguish with a single satisfying provider. The pre-#153 defect #67, kept as the refutation record (the `stages.go` line range below is the pre-fix code): `internal/resolver/stages.go:269-273` built `capProviderIdx[cap.Name] = i` in a nested loop with no guard, so the highest-indexed provider of a capability wins the edge irrespective of version. **Reproduced 2026-08-21** by `TestStage6ProviderSoundness`, which runs all 24 input orders of a four-layer set: a `yy` provider; `mpi@3.0.0` requiring `yy`; `mpi@1.0.0`; a consumer requiring `mpi>=2.0.0`. **Four layers are needed to make R4 falsifiable at all**, and that is the reason this row had no executable basis for as long as it did: with a consumer and two providers the consumer has in-degree 1 and both providers have in-degree 0, so the consumer sorts last whichever provider owns the edge and a wrong edge is invisible. Blocking the *satisfying* provider behind a dependency of its own is what turns the wrong edge into an observable ordering — the consumer mounts before the layer it requires. **12 of the 24 orders reach stage 6** — stage 4's half of #67 rejects the other 12, so one half of this issue masks the other and the reachable count is asserted rather than assumed — **and 3 of those 12 violate R4.** The count is a count of known defects: it goes to zero when #67 is fixed and this test must then be changed on purpose, which is what makes this row dischargeable rather than merely describable. Basis is **`chosen`, not `exhaustive`**: input order is exhausted, and every other dimension — provider count, capability graph shape, constraint form — is a fixture this test picked, which rule 2 says an exhaustive walk of does not establish. Mutation-checked, each count predicted before the run: making stage 6 keep the *first* provider of a capability moves the 3 to 0, and reversing its tie-break (`sort.Ints(queue)` → `sort.Sort(sort.Reverse(sort.IntSlice(queue)))`) moves it to 9. |
| **R5** | *Provider completeness.* If some layer in the resolved set satisfies a consumer's constraint on *c*, resolution does not fail for want of a provider of *c*. | SOUND | exhaustive/implementation — `internal/resolver/provider_matrix_test.go:134 TestStage4ProviderCompleteness` | ENFORCED exhaustive/implementation | **Discharged 2026-09-12 by #153** (merged `9e918de`): `SatisfiesRequirement` (now `spec/layer.go:228`) scans every provider through the shared `Capability.Satisfies` predicate (`:186`), so the 12-of-72 rejections are 0. The load-bearing assertion is no longer the violation count — a reject-nothing implementation also scores 0 — but order-independence of stage 4's verdict, with `wantDiscriminating=12` its non-vacuity bound. The pre-#153 defect #67, same issue as R4's, different half, kept as the refutation record (the `layer.go` line range below is the pre-fix first-match code, at `058e6e1`, #153's parent — the symbol now lives at `spec/layer.go:228`): `spec/layer.go:192-205 SatisfiesRequirement` was version-aware but first-match, so a satisfiable profile is rejected when a non-satisfying provider of the same capability name is encountered first. **Reproduced 2026-08-21** by `TestStage4ProviderCompleteness`, calling the shipping `stage4ValidateGraph` over the full cross product of two `mpi` providers × three versions each × four constraint forms (unconstrained, min, max, both) × both slice orders = **72 cells, of which 12 violate R5** — a satisfiable set rejected for want of a provider it contains. **The declared bound, per rule 2:** the axes are enumerated from `SatisfiesRequirement`'s own inputs rather than picked — a `Provides` list and a `Requirement`, whose only version fields are `MinVersion` and `MaxVersion`, so the four constraint forms are the complete set — while the *values* on the version axis are three majors, chosen because that makes an integer comparison a complete oracle. So the claim is exhaustive over that bound and says nothing about three providers, minor versions, or prereleases. Satisfaction is decided by an oracle written for the table (`satisfiesOracle`), deliberately **not** `semverGTE`/`semverLT`: expectations computed with the code under test hold by construction. The converse is asserted in the same loop — no provider satisfies ⇒ stage 4 must reject — which is what stops R5 being "fixed" by never rejecting anything; mutating stage 4 to accept unconditionally therefore fails on totality rather than passing, and mutating `SatisfiesRequirement` to scan every same-name provider moves the 12 to 0. |
| **R6** | *Environment identity is functional.* `EnvironmentID` is a function of exactly the fields the specification enumerates: changing any enumerated field changes the ID, and changing any non-enumerated field does not. | **TOO WEAK** | withdrawn — R7, X2 | WITHDRAWN (superseded by R7, X2) | **Withdrawn 2026-08-21, not restated — the attempt to restate it is what retired it.** The defect is proof-standard rule 9: *both* of R6's conjuncts defer to an enumeration the implementation owns, so hashing `base_ami_sha256` alone and enumerating `base_ami_sha256` alone satisfies R6 exactly while the identity distinguishes nothing. **What the old statement permitted:** the identity depending on an attestation pointer. Enumerate `rekor_entry`, and re-signing a layer changes `environment_id` — invalidating every cache entry for an environment whose *content* did not change — and old-R6 calls that correct, because the enumeration lists it. **What forbids it now, and why that is not a new R6:** removing the deferral means stating both conjuncts in terms of environment content, and both are already propositions here. Conjunct (a), *changing any enumerated field changes the ID*, becomes *a behaviour that can alter the assembled environment participates in the identity* — that is **X2**. Conjunct (b), *changing any non-enumerated field does not*, becomes *two lockfiles that assemble the same environment have equal `EnvironmentID`* — that is **R7**, verbatim. The enumeration was not incidental to R6; it was the only thing giving R6 content distinct from X2 ∧ R7. R6 in fact became redundant the moment R7 was added (§7 *Propositions restated or added* item 2, which already recorded that “R6 and X2 constrain only the direction behaviour → identity”); the deferral disguised it, and restating R6 is what exposed it. **What could now refute it that could not before — the question that decided withdrawal over restatement:** the one restatement that keeps R6 independent is to move the enumeration into this document — *the ID is a function of exactly these five fields*, `spec/lockfile_hash.go:19-25`. That form is sound, and falsifiable only by **drift between the list here and the struct there**: two representations bound to each other, which is exactly what rule 11 and §6.1 were adopted to stop counting as evidence. It would render `ENFORCED` while asserting nothing about whether either representation is right. A proposition whose only refuting instrument is a consistency check against itself has not moved, so R6 is withdrawn rather than narrowed. **Both citations transfer rather than lapse:** `spec/spec_test.go:542 TestEnvironmentID` asserts that `RekorEntry` does **not** change the ID — that is R7's direction, not functional-on-an-enumeration, so it was always evidence for R7 and never for R6 as written. `spec/packages_test.go:208 TestEnvironmentIDIncludesPackages` witnesses that `Packages` *do* participate, which is X2's direction. R6 carried `ENFORCED E1` on two tests, neither of which tested it. |
| **R7** | *(new)* *No spurious distinctions.* Two lockfiles that assemble the same environment have the same `EnvironmentID`. | SOUND | none | REFUTED (2 of 4 live) | Added 2026-08-21 (§7). R6 and X2 together constrain only one direction — that behaviour reaches the identity. Nothing forbids the identity distinguishing environments that are identical. `OnReady` is hashed (`spec/lockfile_hash.go:23,53`) and executed by nothing (#69), so two lockfiles that differ only in a never-run command list get different IDs and the same environment. **A generator exists and the Basis column is still `none`, deliberately.** `spec/environment_id_r7_fuzz_test.go` searches R7 over an enumerated set of environment-preserving transformations — permuting the layer slice under distinct `MountOrder`, re-signing, varying resolution metadata, appending an advisory `requires_host` entry, rebuilding the `Env` map — justified from the specification rather than from `envHashInput`'s membership, because deferring the domain to the code under test is what made the retired R6 unfalsifiable. 46,546,540 executions on `048aea4` found no failing input. That is evidence for **R7 restricted to that domain**, and R7 as stated quantifies over all environment-preserving pairs, two classes of which still refute it (`OnReady` #69, packages-fetched-at-boot #98) — a third, the `MountOrder`-tie in hand-built lockfiles #95, is discharged 2026-09-14 (`LockFile.Validate()` rejects a tied `mount_order` at every trust boundary, so the slice-order-dependent lockfile is never assembled), and a fourth, the nil-versus-empty inner package slice #117, is discharged by #145 (`omitempty` on `packageSetHashInput.Packages`, `TestEnvironmentID_NilAndEmptyInnerPackagesAgree`). So citing a tier here would be the overclaim this document exists to catch. `TestR7SeedsReachEveryTransform` holds the floor that the seed corpus, which is all `go test ./...` runs, actually reaches every transformation; the search itself needs an explicit `-fuzztime` and runs in `.github/workflows/fuzz.yml`. **Two corrections to the generator's own domain, found after it was written and before it merged.** `vary-attestation` mutated `RekorEntry` and `vary-identity-and-timing` mutated `ProfileName`, on justifications copied from `spec/lockfile_hash.go:14-16`; both fields reach the assembled environment (#120), so R7's premise failed for those pairs and asserting an unchanged identity across them asserted that an X2 violation is *correct*. Both fields are now excluded and the transformations renamed. This is the R6 error one level down — not deferring the domain to `envHashInput`'s membership, but to a comment beside it. **The domain is bounded by the scope clause now stated in §3's preamble:** `internal/overlay/overlay.go:224-231` writes the entire lockfile into the assembled root, so under a literal reading no two distinct lockfiles assemble the same environment and R7's premise would be unsatisfiable; the per-field exclusion of the provenance record (`active.lock.yaml`, `STRATA_REKOR_ENTRY`) is what makes the domain non-empty (#121, resolved). |
...
| # | Proposition | Verdict | Basis | Status | Evidence |
|---|-------------|---------|-------|--------|----------|
| **I1** | *Mount integrity.* Every byte made visible in an assembled environment belongs to a layer whose content hashes to the digest recorded for it in the lockfile. | **TOO STRONG** | withdrawn — I1′, I6 | WITHDRAWN (superseded by I1′, I6) | Satisfying I1 as a universal claim over every visible byte requires abandoning two things Strata deliberately does: `packages:` installs (`internal/agent/agent.go:181`) put bytes from PyPI, conda and CRAN into the merged overlay, and Path B mounts a writable EBS upper (`MutableLayerSpec`). Neither set of bytes belongs to any layer. I1 is therefore restricted to layer-derived bytes, and I6 below covers the remainder. |
| **I1′** | *(restriction of I1)* Every byte made visible **from a layer** hashes to that layer's recorded digest. | SOUND | E1 — `cmd/strata/layer_cache_integrity_test.go:74,123,185,249`, `internal/agent/agent_test.go:181` | ENFORCED E1 | `strata run` route: `cmd/strata/run.go:429` builds the cache path only through `spec.LayerCachePath`, and both return paths hash the file against the declared digest first (`:436`, `:481`). Witnessed by `cmd/strata/layer_cache_integrity_test.go:74 TestLayerCacheAcceptsHonestLayers` (the control, showing the harness can pass) with `:185 TestLayerCacheRejectsPlantedCacheHit`, `:123 TestLayerCacheRejectsTraversalDigest`, `:249 TestLayerCacheRejectsEmptyDigest`. Agent route: `internal/agent/agent.go:240-250` hashes every path `Fetch` returns and compares unconditionally, with no `!= ""` guard; witnessed by `internal/agent/agent_test.go:203 TestRun_SHA256Mismatch`. Domain: these two routes. |
| **I2** | *Cache soundness.* Content obtained from a local cache is used only after its bytes have been hashed and compared against the declared digest, on every use. Under **A2** this must hold for cache hits, not only for fresh downloads. | SOUND | E1 — `cmd/strata/layer_cache_integrity_test.go:74,185` | ENFORCED E1 | Three cache-hit sites, all routed through validation: `cmd/strata/run.go:429` + `spec.VerifyFileDigest`, `internal/registry/s3client.go:369-374`, `internal/registry/localclient.go:271-276`. Witnessed by `TestLayerCacheRejectsPlantedCacheHit` (plants different content under a correct digest and requires refusal) against `TestLayerCacheAcceptsHonestLayers` as the control. `#83` records the standing cost objection — re-hashing every hit is O(environment size) per invocation — and is a design question, not a refutation. |
| **I3** | *Digest well-formedness.* Any value used as a digest — in a comparison, a filesystem path, or a cache key — is syntactically a SHA-256 digest before it is so used. Absent and malformed are both rejected (**A5**). | SOUND | E1 — `spec/digest_test.go:16`, `spec/digest_test.go:53` | ENFORCED E1 | The rule exists and is enforced where it is called: `spec/digest.go:33-50 ValidateLayerDigest` requires exactly 64 lowercase hex and rejects empty explicitly, witnessed at `spec/digest_test.go:16 TestValidateLayerDigest` and `:53 TestLayerCachePathRejectsEscape`. Two sites do not call it. (a) `cmd/strata-agent/s3_fetcher.go:76` builds `filepath.Join(f.cacheDir, layer.SHA256+".sqfs")` from an unvalidated digest and `os.Rename`s into it — #81. (b) `LockFile.IsFrozen()` and `EnvironmentID()` treat any non-empty string as a digest: the repository's own `spec/spec_test.go:544` passes `SHA256: "bbbbbb"` and `internal/resolver/resolver_test.go:597` passes `AMISHA256: "sha256-ami-test123456789"`, and both tests pass. So "frozen" is satisfied by a lockfile with no digests in it. (b) (b) untracked → filed as #96. |
| **I4** | *Path confinement.* No field of a lockfile or manifest can cause a filesystem operation outside the directory designated for it, for any field value. | SOUND | E1 — `spec/digest_test.go:53` | REFUTED (2 of 4 live) | The mechanism is understood and stated correctly in one place — `spec/digest.go:55-58`: *"`filepath.Join` calls `Clean`, which resolves `..` rather than rejecting it, so joining an unvalidated digest can name a file outside `cacheDir`. Every site that builds a layer cache path must go through here."* Executed confirmation: `filepath.Join("/var/cache/strata/layers", "../../../../etc/cron.d/evil.sqfs")` → `/etc/cron.d/evil.sqfs`. Four sites build a path from an unvalidated lockfile field: `internal/trust/verify.go:95` (`layer.ID`, and its own comment at `:90-91` asserts the opposite — #58); `internal/overlay/mount_linux.go:144` (`layer.ID` into `os.MkdirAll` then `MountSquashfs` — a **write** primitive, untracked); `internal/export/oci.go:63` (`"layer-"+lp.ID`, untracked); `cmd/strata-agent/s3_fetcher.go:76` (`layer.SHA256`, #81). Untracked pair → filed as #97. |
| **I5** | *Distinctness.* Two layers with distinct content never share a cache location. | **TOO WEAK** | E1 — `spec/digest_test.go:80` | ENFORCED E1 | Broken-but-satisfying implementation: key the cache on a fresh UUID per fetch. Distinct content never collides, so I5 holds — and nothing is ever a cache hit, because the useful property is the converse one I5 does not state: *the cache location is a function of the content*. Restated, that is what makes a digest a cache key rather than a label. Counterexample to I5 even as written: `cmd/strata-agent/s3_fetcher.go:76` with `layer.SHA256 == ""` yields the filename `.sqfs` for every hashless layer, so distinct content collides (#81). The confined route rejects it — `spec/digest_test.go:80 TestLayerCachePathEmptyDigestDoesNotCollide`. |
| **I6** | *(new)* *Non-layer byte accountability.* Every byte made visible in an assembled environment that does not come from a layer is either (a) produced inside the environment after assembly, or (b) named in the lockfile with a digest that is checked at the moment it is installed. | SOUND | none | REFUTED (2 of 2 live) | Added 2026-08-21 (§7), as the remainder I1 had to give up. `spec.ResolvedPackageEntry` carries a `SHA256` field (`spec/packages.go`), and the installer never passes it: `internal/agent/package_installer.go:104` runs `pip install --quiet <name>==<version>` with no `--require-hashes` (`grep -rn 'require-hashes' . --exclude=PROPERTIES.md` → no output; see §7 item 22), `:115` runs `conda install` and treats a recorded version of `latest` as "whatever is current". The recorded digest is read only by `strata verify --packages` out of band (`internal/packages/resolve.go:275-290`). Refuted for the empty adversary; A3 and A7 make it an attack rather than a hazard. Filed as #98. |
...
| # | Proposition | Verdict | Basis | Status | Evidence |
|---|-------------|---------|-------|--------|----------|
| **T1** | *No unverified mount.* There exists no execution path from *layer declared in a lockfile* to *layer mounted in an assembled environment* that does not pass a successful signature verification under the trust policy in force. | **TOO WEAK** | chosen/implementation @ the `strata run` route — `cmd/strata/run_verify_test.go:370,519`; exhaustive/implementation @ the agent boot route — `internal/agent/boot_matrix_test.go:464` | ENFORCED E1 (weakest of 2 scopes) | *The most important finding in this review.* Broken-but-satisfying implementation: ship an agent whose default policy is `allow-unverified`, and route every layer through a verifier that returns success when a layer names no bundle. Every mount then "passes a successful verification under the trust policy in force" and T1 holds — which is precisely the shape of the five fail-opens this document was written about. T1 defers its content to "the policy in force" and never says what the policy is when the operator selects nothing. It is only worth having when read together with T5's second sentence, and it should be restated to quantify over the *default* configuration (see H1, §1.4). Status under that reading: **both fail-opens now closed (2026-09-13).** The nil verifier was a skip and is now a refusal at `internal/agent/agent.go:302-306` (unless the explicit `Config.AllowUnverified` opt-out — #93, shipped v0.23.0); and at `339329fb`, `:285-293` (layer with `Bundle: ""` dropped from the set; empty set ⇒ success) was **closed by #104**, which replaces the filter with a refusal naming every bundle-less layer at `:310-323`; witnessed at `internal/agent/verify_bundles_test.go:187,252,291` and `cmd/strata-agent/s3_bundle_contract_test.go:22`. Probe 3 on #92 boots READY with a verifier that refuses every artifact, `VerifierCalled=false`. Closed on the `strata run` route by #55: `cmd/strata/run_verify_test.go:519 TestRunRun_MissingKeyIsARefusalNotASkip`, `:370 TestVerifyRunLayers_ReportsEveryFailure`. **Basis raised 2026-08-21 (#129), and it is route-scoped for a reason T1's own text creates.** `TestBootMatrix_DecisionSurface` enumerates all 112 cells of the agent boot route's decision surface — `Config.Verifier` × the bytes a bundle fetch yields × `layer.Bundle` × content-against-`SHA256` — asserting per cell which check fired, whether `Mount` was reached, and how many times the verifier was called. That last is what makes it T1 evidence rather than T5-only: *"`Run` returned nil"* does not distinguish verification succeeding from verification never happening, which is the whole of #92. **#93 closed the last fail-open on this route: `knownOpenCells` is now 0** (`internal/agent/boot_matrix_test.go:99`) — the 20 cells that formerly reached the mount with nothing verified now assert the refusal, pinned as a literal so the hole cannot silently *reopen* without a diff. Two limits on what this cites. (a) It covers the **agent** route; T1 quantifies over *every* execution path, and the `strata run` route is covered separately and only at `chosen`. The Basis cell says so per route rather than in prose, and reduces to the weaker of the two — §2.1 rule 2 and rule 12, the declared bound is part of the claim (#135). (b) The table's expectation ladder makes a boot with no verifier a **refusal**, so it measures T1 under the strengthened reading rule 9 prescribes and not under T1's literal text, which a fail-open still satisfies — the `TOO WEAK` verdict, unchanged. Those 20 cells carried `knownOpen` and asserted the fail-open until #93(a) closed it — the `knownOpen` branch went red once per cell when `Run` began refusing, which was the instruction that closed it — and they now assert the refusal like any other cell. |
| **T2** | *Verification soundness.* Verification succeeds for a layer only if a signature exists, by an identity admitted by the trust policy, over the digest of the exact bytes that will be mounted. | SOUND | E1 — `cmd/strata/run_verify_test.go:466` | REFUTED (1 of 2 live) | The "exact bytes" clause is the load-bearing part and it is satisfied deliberately: `cmd/strata/run.go` adds a check absent from `trust.VerifyLayer` — that the bundle attests *this lockfile's* digest — with the reason stated in its doc comment (*"a missing cosign must not be the difference between 'wrong layer's bundle' and 'accepted'"*). Witnessed by `cmd/strata/run_verify_test.go:466 TestRunRun_RefusesLayerWhoseBundleAttestsAnotherArtifact`. For the *lockfile*, signature verification now exists (#60): `trust.VerifyLockFile` checks a cosign signature over the exact lockfile payload at `strata verify`, `publish` and the agent (v0.26), so the exact-bytes clause holds for the set artifact too. Verification is now mandatory, not opt-in — the #60 partial is discharged: `strata run`, `export`, `fold` (before signing) and the agent all refuse an unsigned or mix-and-match set by default. The row stays live on the identity clause (#225): "an identity admitted by the trust policy" is satisfied here only by possession of one `--key`; `cosign verify-blob` runs no identity or allowed-signers check, so there is no policy to admit or refuse a signer. |
| **T3** | *Transparency binding.* A transparency-log entry accepted as evidence for artifact *A* has a body that corresponds to *A*'s attestation. Under **A4**, the existence of the referenced entry is not itself evidence about *A*. | SOUND | E1 — `cmd/strata/verify_rekor_test.go:80,147,217` | REFUTED (1 of 4 live) | Discharged for the verify command by #59, which replaced a log-index existence check that discarded the bundle: `cmd/strata/verify_rekor_test.go:80 TestVerifyRekorEntries_PassesTheBundle`, with `:217 TestVerifyRekorEntries_BadIndexIsNotVerified` and `:147 TestVerifyRekorEntries_UnfetchableBundleIsAFailure` as the failing controls. The resolver path is no longer a refutation: stage 7 holds a bundle *URI*, not bundle bytes, and the #85 decision (2026-09-14) is that it is not a verification point — it does presence-and-structure only and never accepts a log entry as evidence, so T3 does not apply there. One open objection to the *evidence* remains: #88 — the `hashedrekord` body shape is inferred from our own `Log`, so writer and verifier can be wrong together, which is rule 3's tautology at the level of a design. #86 (`TestStage7_RekorVerification` asserted a negative case its body never exercised, rule 6) is closed 2026-09-14: the test now drives a rejecting client to `REKOR_VERIFICATION_FAILED` and records that stage 7 passes a nil bundle. |
| **T4** | *Trust-anchor independence.* The root of trust used to verify artifacts is not obtained from the authority that serves those artifacts. Under **A1**, an attacker who can replace an artifact cannot also replace the material that decides who may sign it. | SOUND | none | REFUTED (1 of 2 live) | #62 — the agent fetched its cosign public key from the same S3 bucket that served the layers, so A1 alone replaced both — is **discharged** (v0.26): the key is embedded in the binary (`internal/trust`, shared by agent and CLI) and verified to be the public half of the KMS signing key. #63 — `ec2runner` downloads the cosign binary itself with no checksum or signature check, which is the §1.2 clause "obtaining the verification binary is in scope under A3" paying out — remains the live half. |
| **T5** | *Fail-closed.* Any inability to complete verification — absent tool, absent key, absent bundle, absent log entry, network failure, unparseable material — results in refusal. Degradation to a weaker check occurs only when a weaker policy has been explicitly selected by the operator. | SOUND | chosen/implementation @ the `strata run` route — `cmd/strata/run_verify_test.go:264`; chosen/implementation @ verifier construction — `cmd/strata-agent/cosign_verifier_test.go:127,187,249`; exhaustive/implementation @ the agent boot route — `internal/agent/boot_matrix_test.go:464` | ENFORCED E1 (weakest of 3 scopes) | The strongest proposition in this document: it names absence alongside invalidity, so A5 cannot slip past it, and it fixes the default in its second sentence, which is what T1 fails to do. Was refuted at `internal/agent/agent.go:288` (#93) and, at `339329fb`, `:285-293` (#92) — an absent bundle was a skip, not a refusal. **Both halves are now closed (2026-09-13):** #104 refuses an absent bundle at `:310-323` (witnessed at `internal/agent/verify_bundles_test.go:187,252,291` and `cmd/strata-agent/s3_bundle_contract_test.go:22`), and #93 (shipped v0.23.0) makes a nil `Verifier`/`BundleFetcher` a refusal at `:299-303` unless the explicit `Config.AllowUnverified` opt-out is set. Enforced at E1 on the routes already fixed: `cmd/strata/run_verify_test.go:264 TestNewRunVerifier_NeverReturnsANilVerifier` (#55) and `cmd/strata-agent/cosign_verifier_test.go:127 TestResolveVerifier_NilVerifierOnlyWithAnExplicitOptOut`, `:187 TestAllowUnverified_DefaultsToClosed`, `:249 TestProductionPrereqs_ClosedByDefault` (#56). **Basis raised 2026-08-21 (#129), and scoped per route 2026-08-22 (#135): the cell now reduces to `chosen/implementation`, the weakest of its three routes, rather than reporting the strongest.** T5's list of failure modes — *absent tool, absent key, absent bundle, absent log entry, network failure, unparseable material* — is close to an enumeration of a domain, and `TestBootMatrix_DecisionSurface` now runs one: the bundle-bytes dimension covers no fetcher, valid-for-this-content, valid-for-*other*-content, `(nil, nil)`, non-JSON, well-formed JSON with the wrong media type, and a fetch error, crossed with four verifier dispositions, `layer.Bundle` empty or named, and content matching or not matching its declared `SHA256` — 112 cells, each asserting *which* check fired rather than merely that one did. Where T5's own list stops short of the shipping surface, the enumeration is derived from the code's inputs and covers more (valid-for-other-content is a substitution, not an inability), and where it goes beyond this route it is not covered here at all: **an absent cosign binary and an absent Rekor entry are neither of them dimensions of this table**, so T5's "absent tool" and "absent log entry" clauses remain at `chosen` on the routes cited above. **#93 closed the nil-`Verifier`/`BundleFetcher` fail-open on this route: `knownOpenCells` is now 0** (`internal/agent/boot_matrix_test.go:99`) — the 20 cells that formerly reached the mount unverified now assert the refusal, and fail if it silently reopens. That was the last live half keeping T5 refuted on the covered routes. |
| **T6** | *Policy explicitness.* A lockfile carries a field naming the trust policy under which it was produced — what the resolver checked about its layers' attestations — and a consumer reading only the artifact can determine it, without knowledge of the invoking environment. | SOUND | E1 — `internal/resolver/verification_policy_test.go` | ENFORCED E1 | **Restated 2026-09-14 from `ILL-FORMED`.** The original — *"recorded alongside that result"* — named neither an artifact nor a lifetime: a line on stderr satisfied it on one reading and no durable record on another, so it could not be failed. The restated form names the artifact (the lockfile) and the reader (a consumer of the artifact), and is refuted by absence — which is how it stood until 2026-09-14, when `grep -rn 'TrustPolicy\|VerifiedAt'` returned nothing and no lockfile field recorded what was checked. **Discharged (#100):** `LockFile.VerificationPolicy` (`spec/lockfile.go`) records `attestation-references-present` or `unsigned-offline`, set by the resolver from what stage 7 checked (`internal/resolver/stages.go`); a resolve holds bundle URIs not bytes, so it never claims log verification (#85), and the label was renamed from `attestation-present` (post-release review) so it cannot be read as "cryptographically verified" — it attests that the *references* are present. Its route out of the identity is `inert` — recorded for audit, read by no assembler — asserted by `TestEnvironmentIDRoutes_MatchBehaviour`. E1 at `internal/resolver/verification_policy_test.go`: signed layers record `attestation-references-present`, an unsigned layer accepted offline records `unsigned-offline`. **What makes the label meaningful rather than green-by-construction:** the review noted `verificationPolicy` computes the label from the same fields it summarises, so the discharge also rests on the *enforcement* that the label reports — in strict mode a layer missing its bundle or Rekor entry is refused, not labelled, and that refusal is `TestStage7_BundleMissing`/`TestStage7_RekorEntryMissing`; deleting the stage-7 check reddens those, so "references present" is a claim about a check that ran, not only a value computed from fields. |
| **T7** | *Command-name honesty.* A command's name and documented behaviour do not claim a stronger check than it performs. A flag that purports to disable a check disables a check that was otherwise performed. | **TOO WEAK** | E1 — `cmd/strata/run_verify_test.go:350,497` | REFUTED (1 of 4 live) | Broken-but-satisfying implementation: keep the fail-open and fix the *documentation*. T7 constrains names and docs, so the cheapest way to satisfy it is to document the weakness while leaving the command called `verify` and the behaviour unchanged — the operator's attention is displaced exactly as before. Repair: pair T7 with a requirement that a weaker check announces itself **at use time**, not only in prose. `strata run --no-verify` already meets the repaired form (`cmd/strata/run.go:187` prints the warning; `cmd/strata/run_verify_test.go:350 TestVerifyRunLayers_NoVerifyAnnouncesTheSkip` with `:497 TestRunRun_NoVerifyReachesTheMount` as its pair), and so does `STRATA_AGENT_ALLOW_UNVERIFIED` (#56). `strata verify` does not: without `--rekor` it performs presence checks only (`cmd/strata/verify.go:200-216 collectPresenceFailures`) under a name that claims verification — #60. And `verifyBundles`'s doc comment stated the skip as intended design (`internal/agent/agent.go:275-286`) — T7's broken-but-satisfying implementation occurring in the wild rather than as a hypothetical: the weakness was documented accurately and the behaviour left alone, so T7 as written was *satisfied* by the artifact that recorded the fail-open. #104 rewrites the comment and inverts the behaviour together, which is the only combination that discharges anything. |
| **T8** | *(new)* *Freshness.* A verifier can determine that the artifact set it has been given is the current one, and refuses a set that is older than a stated bound. | SOUND | none | REFUTED | Added 2026-08-21 (§7). §5 asked for group T to be checked against TUF's taxonomy; **A6 was in the model and no proposition used it.** Doing the check: nothing records a freshness threshold, an expiry, or a timestamp/snapshot role, so rollback and freeze both succeed against a genuinely signed old artifact, and `strata update` moves forward only when a human runs it. Filed as #101. **Update 2026-09-16 (v0.28.0, #224):** a freshness bound now exists — the staleness is surfaced on every consumption path and refused opt-in via `--max-age` / `STRATA_AGENT_MAX_AGE` (`spec/freshness.go`). T8 stays REFUTED (partial): the refusal is opt-in by design (a default would refuse a frozen, cited lockfile) and there is still no signed-timestamp/snapshot role. See the §4 #224 row. |
| **T9** | *(new)* *Set integrity.* The layer set is verified as a set, not layer by layer: a verifier refuses a combination of individually valid layers that was never attested together. | SOUND | E1 — `cmd/strata/run_setverify_test.go` | ENFORCED E1 | Added 2026-08-21 (§7). This is TUF's mix-and-match attack. The lockfile is the artifact that names the set, and it is now signed as a whole (`trust.SignLockFile`, v0.26) and verified at `strata verify`, `publish` and the agent, so a set of individually-signed layers that no maintainer attested together has no valid lockfile signature and is refused where verification runs. Verification is now mandatory (#101, v0.26): `strata run` (`verifyRunSet`), `strata export`, `strata fold` before signing, and the agent all verify the set before consuming it and refuse an unsigned or mix-and-match combination by default, with `--no-verify` the explicit announced escape (as for per-layer verification, #55). With the #60 counterexample discharged and no other live ground, T9 is enforced; the freshness dimension is T8/#224, not set integrity. |
...
refuted. Extraneous dependencies → R4, discharged 2026-09-12 (#67, #153). Endless data → not covered
...
| # | Proposition | Verdict | Basis | Status | Evidence |
|---|-------------|---------|-------|--------|----------|
| **P1** | *Publication precondition.* A persistent identifier is minted only for a lockfile that is fully frozen, contains no mutable layer, and has been verified under a stated trust policy. | **TOO WEAK** | none | REFUTED (2 of 3 live) | Broken-but-satisfying implementation: define `IsFrozen()` as `return true`. P1 defers "fully frozen" to a predicate the implementation owns — the third instance of this pattern, with R2's exception clause and R6's enumeration. And the deferral is not hypothetical: `IsFrozen()` (`spec/lockfile.go:114-121`) inspects layer digests and the base AMI digest and **ignores `Packages` entirely**, so a lockfile that will `conda install` a `latest` version at boot is "fully frozen". Status: `cmd/strata/publish.go:76` checks `IsFrozen()` and nothing else — not `IsSigned()`, not `!HasMutableLayer()`, no trust policy (#66). |
| **P2** | *Independent verifiability.* Given only the published record, a third party with no access to the original registry, no shared secrets, and no contact with the publisher can establish the authenticity of the environment it describes. | SOUND | none | REFUTED | The claim the whole project rests on. The deposit now carries a verification bundle, not the lockfile alone (#99, v0.26): `internal/zenodo/zenodo.go` `Deposit` uploads the cosign public key (`cosign.pub`) and a `STRATA-VERIFY.md` manifest alongside the lockfile, whose inline `bundle` field is the signature over the whole layer set. A third party holding only the record can verify the environment was attested as a unit by the signing key, against the deposited public half of that key (the embedded anchor, #62/#222) — the digests are bound, not dangling. The row stays live on position (a) of #99: layer *bytes* and the per-layer attestation bundles are not deposited (too large for a citation record), so byte-level confirmation still requires fetching from the registry named in each layer's `source`, with the signed digest as the binding. See the #99 row. |
| **P3** | *Referent stability.* The environment identity attested by a published record cannot subsequently denote different bytes. | SOUND | none | REFUTED (2 of 2 live) | `internal/agent/package_installer.go:118` treats a recorded conda version of `latest` (or empty) as "resolve at boot", and `:101` installs pip packages from PyPI with no hash, so one `EnvironmentID` denotes different bytes on different days. Refuted for the empty adversary; A7 makes it steerable. Same root as I6; filed together as #98. Second instance, measured on `93df7ca` and structurally different: the identity is a *storage key*. `PutLockfile` writes `locks/<environmentID>.yaml` unconditionally in both clients, so two lockfiles that share an `EnvironmentID` and differ in content cannot coexist — the second silently replaces the first, and `strata-agent` boots whichever was published last from the URI the caller was told to tag the instance with. A published record's referent therefore changes on someone else's publish, with no adversary and no error (#124). |
| **P4** | *Freeze attainability.* A lockfile produced by ordinary resolution can satisfy the system's own definition of frozen, without manual editing. | SOUND | none | REFUTED (1 of 3 live) | #64 — `strata freeze` structurally cannot succeed because nothing populates `ami_sha256`, and `IsFrozen()` requires `Base.AMISHA256 != ""`. The draft's own note anticipated the consequence and it holds: P1 is presently vacuous, because the precondition P1 guards cannot be reached by ordinary output. |
...
| # | Proposition | Verdict | Basis | Status | Evidence |
|---|-------------|---------|-------|--------|----------|
| **B1** | *Artifact identity.* The digest recorded for a built artifact is the digest of the bytes distributed under that identity. | SOUND | E0 — `internal/build/pipeline.go:171,199,215` | ASSERTED (E0) | Holds by construction on the one build path: `internal/build/pipeline.go:171` hashes `sqfsPath`, `:196` signs the same `sqfsPath`, `:212` pushes the same `sqfsPath` — one file object throughout, no re-generation between hash and push. E0 rather than E1 because the only test that could witness it end to end is `internal/build/pipeline_integration_test.go`, which carries a `//go:build` tag and is therefore not run by CI's `go test -race ./...` (`.github/workflows/ci.yml:25`, no `-run` filter, `ubuntu-latest`). Rule 4. Footnote: `pipeline.go:55` writes the literal `"dry-run"` into `manifest.SHA256` on the dry-run path, which is an I3 instance confined to a path that pushes nothing. |
| **B2** | *Input completeness.* Every input consumed by a build is named in the recipe together with a digest, and the build fails if a fetched input does not match. Under **A3**, an unpinned fetch is attacker-controlled. | SOUND | none | UNPOPULATED | #68, confirmed by command: `internal/build/recipe.go`'s `RecipeMeta` has **no source or digest field** at all (`grep -n 'yaml:' internal/build/recipe.go`), and recipes fetch by piping straight into tar — e.g. `cmd/strata/recipes/core/R/4.5.2/build.sh:7`, `curl -fsSL "${URL}" \| tar -xz`. There is nothing to compare against and no place to record it. |
| **B3** | *Build reproducibility.* Re-executing a recipe against its recorded build environment yields a bit-identical artifact. | SOUND | none | UNPOPULATED | Refuted by B2 before any measurement: an unpinned `curl` means the same recipe consumes different bytes over time, so bit-identity cannot hold across the interval in which upstream changes. The claim is nonetheless asserted in the code — `internal/build/recipe.go:280-281`, *"Same recipe + same build environment = same SHA256"* — and the mksquashfs flags that would support it are real (`-mkfs-time 0`, `-all-time 0`). Had B2 held, the status would still be E0: the only test that compares two builds is `internal/build/squashfs_integration_test.go`, build-tagged and therefore unexecuted (rule 4). |
...
| # | Proposition | Verdict | Basis | Status | Evidence |
|---|-------------|---------|-------|--------|----------|
| **X1** | *Specification completeness.* Every behaviour the profile schema permits an author to specify is either executed by the runtime or rejected at parse time. A field that is accepted, recorded, and never acted upon is a defect. | SOUND | none | REFUTED (2 of 5 live) | Instances, each named with the issue it is filed as. No count is restated here: the register rows are the source and `Status` derives the total, and the lettering does not map one-to-one onto rows — `Profile.Instance` and `Profile.Storage` are two instances under one issue. (a) `OnReady` — parsed at `spec/profile.go:52`, copied into the lockfile at `internal/resolver/stages.go:437`, hashed at `spec/lockfile_hash.go:53`, executed nowhere (#69). (b) `Profile.Instance` and (c) `Profile.Storage` — `grep -rn 'InstanceConfig\|StorageMount' --include='*.go' . \| grep -v _test.go` returns only the declarations at `spec/profile.go:42,45,313,331` and no use; both are parsed, dropped at resolution, and never reach the lockfile. (d) `ResolvedPackageEntry.SHA256` — recorded and hashed into the identity, and never used by the component that installs the package (see I6). `RequiresHost` is a further candidate and is exempted only because the schema says so in as many words — *"Advisory only in v0.21.0"* (`spec/lockfile.go:60-63`), which is X1's rule applied honestly rather than a violation of it. (e) `validated_on` — was a verification claim no code read; **removed 2026-09-13 (#113)**, X1's remedy for an unacted field, so this instance is discharged. (f) the twelve `pending-initial-build` placeholders occupying `rekor_entry` and `bundle` (#46, added below) — **signed 2026-09-17 (#237, v0.29), so this instance is discharged**: `strata sign-formation` replaced every placeholder with a real Rekor attestation. Filed as: (a) #69, (b) and (c) #102, (d) #98, (e) #113, (f) #46. |
| **X2** | *Identity captures behaviour.* If a specified behaviour can alter the state of the assembled environment, that behaviour's specification participates in the environment identity — or the identity does not determine the environment. | SOUND | none | REFUTED (1 of 4 live) | The one live instance is the converse-direction refuter that hashing cannot fix: `Packages` *do* participate in the identity and still fail to determine the environment, because the bytes they name are fetched from a moving upstream at boot — conda `latest` resolves at boot and the `pip`/`conda` install ignores the recorded digest (I6, P3, #98). **The three instances that were bare absences from `envHashInput` are discharged by #145** (merged `69f789d`), which widened the identity to the fields the assembler reads — see the register rows for the re-derived evidence: `LockFile.Defaults`, which writes `/etc/profile.d/strata-defaults.sh` (#118); `ProfileName`, exported into the assembled root and the child environment (#120); and per-layer `InstallLayout`/`Name`/`Version`, which build `PATH`/`LD_LIBRARY_PATH` (#122). Each is now hashed, with `TestEnvironmentIDRoutes_MatchBehaviour` asserting participation field-by-field and `TestEnvironmentIDRoutes_CoverEveryField` failing the build if a field escapes classification. `RekorEntry` — the other half of #120 — is *legitimately* outside the identity: it takes the declared-provenance route excluded by name in §3's scope clause (#121), enforced by `TestDeclaredProvenance_NoInTreeReader`, not asserted. **The godoc that asserted X2 as fact is corrected** — `spec/lockfile.go:144-154` no longer reads "Two lockfiles with the same EnvironmentID describe identical environments" but "were assembled from identical instructions", with an explicit pointer that the stronger claim is REFUTED here (#123, also #145). **And the destructive consequence is now closed at the publish boundary:** the identity is a registry storage key, and a colliding or empty-id publish refuses loudly rather than silently overwriting it (#124, PR #174 — local exclusive-create `O_EXCL`, S3 `IfNoneMatch`), which was the cheapest mitigation for the whole class — loud at publish, without waiting for the identity question to be settled. A writable `MutableLayer` upper alters the assembled state and is deliberately *excluded* as non-content-addressed (`spec/lockfile_hash.go:17-18`); that is a standing tension X2 names but no register row counts, because the exclusion is declared rather than accidental. |
| **X3** | *(new)* *Documented-form acceptability.* Every profile form the documentation presents as valid is accepted by the parser, and every artifact the documentation presents as runnable resolves against the shipped catalog. | SOUND | E1 — `spec/softwareref_yaml_test.go:22-24`, `spec/docsnippets_test.go` | ENFORCED E1 | Added 2026-08-21 (§7) because two tracker issues refuted nothing in §3 and both were the *converse* of X1. X1 lets a system satisfy it by rejecting everything at parse time; nothing said the documented forms must be accepted. #53 — the inline software-ref form `- python@3.13`, presented in the documentation, did not parse because `SoftwareRef` had no `UnmarshalYAML`; discharged at E1 by `spec/profile.go:240` with `spec/softwareref_yaml_test.go:22-24` and `spec/docsnippets_test.go`, the latter testing the documentation sites themselves. #70 — **discharged 2026-09-13**: all three examples named `@2024.03` formations the catalog (only `@2026.03`) did not contain and standalone layers with no recipe, so `examples/` had a 0% resolve rate while four parse-only tests (`examples/examples_test.go:21,48`, `examples/catalog_test.go:19,101`) passed — rule 6 in the documentation dimension. Fixed: versions bumped, unrecipe'd layers removed, and `TestShippedExamplesResolveOffline` now resolves every example against the shipped catalog. With #53 and #70 both discharged X3 is enforced on the routes cited — its documented forms parse and its shipped artifacts resolve. |
...
| I6 | pip SHA256 pins were validated against nothing | A3 | #51 | Partially — `strata verify --packages` validates the recorded pin against PyPI out of band (`internal/packages/resolve.go:275-290`), **held by reading**: it needs live PyPI, so there is no unit test and this half is not evidenced at `chosen`/implementation. The *install* still ignores the pin (see the I6 row below), which is the live half. `Partially` counts as live |
| T7 | `strata run` did not warn that `packages:` entries are unattested | H1 | #48 | No — **re-derived 2026-08-22** under §2.1 rule 11; was `Yes — closed completed`, which is the closure-for-evidence conflation that rule names (#137). The row and the code answer two different questions. #48 asked for a warning that the packages *will not be installed* by `strata run`; that shipped at `cmd/strata/run.go:95-108`, counts entries rather than sets, and names both the installer and the consequence — E1 at `cmd/strata/run_packages_warning_test.go:78,132`. The row says "unattested", a claim about the attestation chain. As of v0.27 the **agent** discloses it before installing (`unattestedPackagesWarning`, `internal/agent/agent.go`, written to the boot log) and **`strata verify`** reports it (`packagesUnattestedNote`, `cmd/strata/verify.go`) — the two routes where an operator can act, closing #139. (The recorded `SHA256` is also now *enforced* when present rather than ignored — #98 — so these entries are no longer wholly outside verification, only outside the layer *signature* chain.) The `strata run` route — this row's literal subject — stays silent: it does not install these entries, `cmd/strata/run_packages_warning_test.go:156` pins that silence, and the wording repair is #140. So the row stays `No` on the run route specifically, while the substantive disclosure #139 asked for now exists. Rewording a counterexample in the change that discharges it is narrowing-until-satisfied, so the run wording still travels alone (#140) |
| T7 | The resolver expanded unattested formations without warning | H1 | #49 | Yes — **discharged 2026-09-14 (#138)**; warning existence re-derived 2026-08-22 under §2.1 rule 11 (was `Yes — closed completed`, #137). The warning exists and fires, naming the formation, the reason and the placeholder value (`internal/resolver/stages.go:74-78`) — E1 at `internal/resolver/formation_attestation_warning_test.go:132`, with `:159` the control that an attested formation is *not* warned about. All four resolver constructions now deliver it: the three CLI paths (`cmd/strata/resolve.go:66`, `update.go:49`, `freeze.go:34`) set `Warnings: os.Stderr`, and the fourth — the `pkg/strata` library route, previously the live half — now threads a caller-supplied writer: `pkg/strata.Options.Warnings` (`io.Writer`) flows into `resolver.Config.Warnings` (`pkg/strata/strata.go`), re-derived at E1 by `pkg/strata/warnings_test.go` (`TestResolve_SurfacesPlaceholderAttestationWarning` asserts a formation carrying `pending-initial-build` surfaces the warning through the library; `TestResolve_WarningsNilByDefaultDoesNotPanic` is the control that a nil writer is tolerated — no default stderr, library etiquette). The fix is an opt-in at the library boundary, not a change to `resolver.warn`'s nil contract, so `internal/resolver/formation_attestation_warning_test.go:193` (resolver-level silence on a nil writer) correctly stays green. T7 stays `REFUTED` on #48 (#139) and #60 |
...
| X3 | All three shipped examples name formation versions the catalog does not contain, and `go test ./examples/` passes | H1 | #70 | Yes — the examples resolve. **Re-derived 2026-09-13** at E1 (chosen/implementation): `cmd/strata/formations_resolve_test.go`'s `TestShippedExamplesResolveOffline` resolves every `examples/*.yaml` against the embedded catalog and asserts a lockfile — the cross-against-the-catalog check the parse-only `examples/*_test.go` never did. The stale `@2024.03` formation refs are now `@2026.03`, and standalone layers with no recipe (`alphafold`, `pytorch`, `texlive`, `git`) that stage 3 rejected as `LAYER_NOT_FOUND` are removed; each example is now its formation plus recipe-backed software. Offline resolvability itself is #108 |
| X3, P4 | None of the six shipped formations resolves against the shipped catalog, at the versions that do exist. Measured on `87f02fe`: 6 of 6 exit 1, 0 lockfiles produced — five at stage 7 `BUNDLE_MISSING`, and `hpc-mpi@2026.03` at stage 4 `UNSATISFIED_REQUIREMENT`. That last is **four** missing layer dependencies, not one: `openmpi`'s recipe declares `runtime_requires` on `ucx@>=1`, `hwloc@>=2`, `pmix@>=5` and `libfabric@>=1`, all four shipped as recipes and none listed in the formation, which lists only `gcc` and `openmpi`. Stage 4 returns on the first, so the reported `ucx` understates it — enumerated by probe against `spec.BaseCapabilities.SatisfiesRequirement`, the predicate `stage4ValidateGraph` itself uses (originally worded *"because `openmpi` requires `ucx@>=1` and the formation omits a `ucx` layer"*; corrected 2026-08-21). Distinct from #70: these profiles name `@2026.03`, which exists. The one CI job that resolves end to end points at the `file://` fixture, where bundles exist by construction, and its profile names layers rather than formations | H1 | #108 | Yes — the embedded offline catalog now resolves. **Re-derived 2026-09-13** at E1 (chosen/implementation): `cmd/strata/formations_resolve_test.go` `TestShippedFormationsResolveOffline` resolves all six shipped formations against the embedded catalog and asserts a lockfile per formation, and the strict path stays refused — forcing `AllowUnsignedOffline` off makes all six fail `BUNDLE_MISSING`, `hpc-mpi` now at **stage 7, not stage 4**, which is the ucx/hwloc/pmix/libfabric fix showing. Two parts: stage 7 accepts an unsigned layer **only** on the no-registry offline path (`resolver.Config.AllowUnsignedOffline`, set by `cmd/strata/resolve.go`'s `offlineCatalogOnly`, warned once in `stage7VerifyBundles`), and `hpc-mpi@2026.03` gained `openmpi`'s runtime deps so it passes stage 4. Does **not** discharge X3 (#70 stays live) or P4 (#64 stays live — the produced lockfile resolves but is unsigned and unfrozen, which is P4's freeze story, untouched here) |
| X1 | `validated_on` is a verification claim that no code reads and that is false where it can be checked. `grep -rn 'ValidatedOn' --include='*.go' .` returns its declaration and doc comment at `spec/layer.go:226,228` and nothing else — no reader in non-test code and none in tests. All six shipped formations assert it. For `hpc-mpi@2026.03` the assertion cannot ever have held: its four missing dependencies fail `stage4ValidateGraph`, which checks requirements against base capabilities plus *the resolved set only* and never adds a layer to satisfy one, and stage 4 (`internal/resolver/resolver.go:122`) runs before stage 7 (`:135`) — so the failure is independent of bundle, registry and attestation state, and no registry serving the shipped `openmpi` recipe resolves it | H1 | #113 | Yes — **removed 2026-09-13**. `validated_on` was accepted, recorded, and read by nothing — X1's exact defect — so X1's own remedy applies: delete the unacted field rather than invent a reader. Gone from the `Formation` struct in `spec/layer.go` and from all six shipped formations (e.g. `cmd/strata/formations/hpc-mpi@2026.03.yaml`). **Re-derived** exhaustive/implementation: `grep -rn ValidatedOn --include=*.go .` and a grep for `validated_on` across `cmd/strata/formations/` both return nothing. `hpc-mpi`'s structurally-false assertion is moot twice over — the claim is gone, and #108 made the formation resolve. X1 stays refuted by #69/#102/#98/#46 |
| X1 | Twelve `pending-initial-build` values across the six shipped formations occupy `rekor_entry` and `bundle` — fields whose names assert an attestation that does not exist. The resolver responds with `r.warn` (`internal/resolver/stages.go:74-78`, the deliberate outcome of #49) and never propagates either field into a lockfile: `stages.go:409-413` copies `FromFormation`, the formation *name*, alone. So `internal/agent.verifyBundles` cannot see the placeholder and nothing downstream can act on it. Recorded here for the first time on 2026-08-21; previously the register mentioned #46 only in §4.1's "same category" list, which is the silent state the register exists to prevent | H1 | #46 | Yes — **2026-09-17 (#237, v0.29)**. The twelve placeholders are replaced with real attestation: `strata sign-formation` (`trust.SignFormation`, the formation analog of the v0.26 lockfile set-signing) signs each shipped formation as a unit with the KMS key and logs it to Rekor. All six shipped formations now carry a real `rekor_entry`/`bundle`/`signed_by` — e.g. `genomics-python@2026.03` at Rekor log index 2879086085 — rather than `pending-initial-build` (`grep -L pending-initial-build cmd/strata/formations/*.yaml` returns all six). **Re-derived** at E1 (chosen/implementation): `internal/trust/formation_test.go` (round-trip, tamper, rekor-entry mismatch, unsigned) and `cmd/strata/sign_formation_test.go` (the placeholder is replaced and the result verifies). The fields no longer assert an attestation that does not exist. Resolve-time *verification* of the formation signature (a consumer calling `trust.VerifyFormation`) is a distinct follow-up, not this placeholder counterexample. |
| R4, R5 | Stage 4 validates against one provider, stage 6 wires the edge to another; stage 4 is itself first-match. **Boundaries measured 2026-08-21** (`internal/resolver/provider_matrix_test.go`): the stage-4 half rejects 12 of 72 satisfiable provider/constraint/order cells; the stage-6 half misorders 3 of the 12 input orders that reach it, the other 12 of 24 being rejected by the stage-4 half first. Recorded here because a counterexample with no reproduction cannot be discharged — only closed | H1 | #67 | Yes — #153, merged `9e918de`. **Re-derived 2026-09-12**, all three tests green at that head (§2.1 rule 11: the citation is the re-derived evidence, not the closed issue). The two rules are now one predicate, `spec.Capability.Satisfies` (`spec/layer.go:189`), so stage 4 and stage 6 can no longer disagree. R5, exhaustive/implementation: `internal/resolver/provider_matrix_test.go:134 TestStage4ProviderCompleteness` — 12 of 72 → 0, with order-independence (`wantDiscriminating=12`) now load-bearing rather than the count, since a reject-nothing implementation also scores 0. R4, chosen/implementation: `:302 TestStage6ProviderSoundness` — 3 of 12 → 0 over the same 24 orders — and `:453 TestStage6SelectsHighestSatisfyingProvider` pins that the edge follows highest satisfying version (tie-break lowest layer ID), not slice order, which the soundness table with one satisfying provider cannot distinguish. `BaseCapabilities.HasCapability` (`spec/layer.go:218`) had the same first-match shape with no shipped caller and was fixed in the same commit |
| T1, T5 | `verifyBundles` skips when the verifier is nil **and** when a layer names no bundle | A1 + A5; H1 for the nil half | #93 (fix); the decision half was #92, closed | Yes — both halves closed. **Absent-bundle half**: #104, refusal at `internal/agent/agent.go:313-326`, E1 `internal/agent/verify_bundles_test.go:187,252,291`. **Nil-verifier half**: #93, shipped v0.23.0 — `verifyBundles` refuses when `Verifier` or `BundleFetcher` is nil unless the explicit `Config.AllowUnverified` opt-out is set (`internal/agent/agent.go:302-306`), which only `cmd/strata-agent` sets and only under `STRATA_AGENT_ALLOW_UNVERIFIED`. **Re-derived 2026-09-13** at exhaustive/implementation on the agent boot route: `internal/agent/boot_matrix_test.go` enumerates all 112 cells of the decision surface and `knownOpenCells` is now **0** (`:99`) — the 20 cells that formerly reached the mount unverified now assert the refusal, and the `knownOpen` branch went red once per cell when `Run` began refusing (its own instruction closed #93(a)). Green at `6413ce3`. The `strata run` route is covered separately at E1 (#55). Content-SHA256 integrity was never the gap; authenticity was, and it is now fail-closed by default |
...
| T2, T7, T9 | `strata verify` and `IsSigned()` are presence checks; no lockfile verification exists | A1 | #60 | Yes — **2026-09-15 (v0.26)**. Lockfile verification now exists, is real, and is mandatory: `trust.VerifyLockFile` (`internal/trust/lockfile.go`) checks a cosign signature over the whole layer set, and `strata verify` calls it against the public key embedded in the binary (`internal/trust/anchor.go`), so a mix-and-match of individually-valid layers is caught (demonstrated end-to-end: `strata sign` against the KMS key logs to Rekor, `strata verify` confirms with no AWS, and tampering a layer digest fails the check). `trust.SignLockFile` + `strata sign` produce the signature. `strata verify`, `strata publish`, and the EC2 agent (`internal/agent.Agent.Run`, before fetch/mount) all now verify the set signature when the lockfile carries one — publish refuses a *tampered* lockfile before minting a permanent DOI, and the agent refuses to boot one, which the old `IsSigned()` presence check could not catch. **Re-derived** at E1 (chosen/implementation): `internal/trust/lockfile_test.go` (round-trip + tamper/mix-and-match/unsigned), `cmd/strata/sign_test.go`, `cmd/strata/publish_test.go` (rejects unsigned and invalid-signature), `internal/agent/agent_lockfile_sig_test.go` (boots a valid one, refuses a tampered one before any fetch/mount). Verification is now **mandatory**, not opt-in, on every consumption path: `strata run` verifies the set before mounting (`verifyRunSet`, `cmd/strata/run.go`), `strata export` verifies before packaging an image, and `strata fold` verifies its inputs before signing a merged layer — refusing an unsigned or mix-and-match set by default. `--no-verify` (and fold's `--no-sign`/`--eject`) are the explicit, announced escapes, exactly as `--no-verify` already is for per-layer verification (#55, discharged). **Re-derived** at E1: `cmd/strata/run_setverify_test.go` (the set signature gates the mount; an unsigned set and a layer appended after signing are both refused). Two grounds are tracked separately and are *not* this counterexample: T2's remaining ground is that there is no signer-identity policy (#225), and T8's freshness bound is still open (#224). |
| T2 | verification admits any holder of the configured `--key`; there is no signer-identity policy to admit or refuse a signer | A1 | #225 | No — the exact-bytes and signature-required clauses of T2 hold and verification is now mandatory (see the #60 row), but *"an identity admitted by the trust policy"* is satisfied only by key-possession: `cosign verify-blob --key <public key>` runs no identity or allowed-signers check, so anyone presenting a signature under that one key is admitted and nothing can refuse a signer. Recorded as a distinct ground so discharging the "no verification exists" counterexample above is not read as T2 fully enforced. |
| T3 | `LockFile.RekorEntry` (published as `lockfile_rekor_entry` in `ProvenanceRecord`) was outside the signed payload and unchecked against the bundle, so a false transparency pointer verified with "signature verified" | A1 | #220 | Yes — **2026-09-15**, found in the v0.26 re-review. `RekorEntry` cannot be inside the payload it points at, so `VerifyLockFile` (`internal/trust/lockfile.go`) now binds it: it requires the bundle to carry a transparency-log entry and rejects `lf.RekorEntry != bundle.RekorLogIndex()`, and `SignLockFile` refuses a bundle with no tlog entry rather than letting a stale value survive. Not a mix-and-match bypass — the genuine bundle still authenticates the set — but it stops a falsified provenance citation from shipping as verified. **Re-derived** at E1 (chosen/implementation): `internal/trust/lockfile_test.go` `TestVerifyLockFile_RejectsRekorEntryMismatch` (signs, alters only `rekor_entry`, requires failure) |
| T4 | Agent fetches its cosign public key from the bucket that serves the layers | A1 | #62 | Yes — **2026-09-14 (#62)**. The agent's trust anchor is pinned into its binary at build time (`cmd/strata-agent/keys/cosign.pub` via `//go:embed`), not fetched from the registry: `productionPrereqs.fetchKey` is now `writeEmbeddedCosignKey`, which materialises the embedded bytes to a temp file, and the S3 `fetchPublicKey` is removed. An actor with write access to the layer bucket (A1) can no longer substitute the verification key, and deleting it is impossible — the key ships in the binary. The fail-*open* on a missing key was already closed (#56, `resolveVerifier` refuses to boot); #62 closes the key's *origin*. **Re-derived** at E1 (chosen/implementation): `cmd/strata-agent/embedded_key_test.go` `TestEmbeddedCosignKey` asserts the embedded key is non-empty and PEM-parseable — so a release cannot ship an empty anchor unnoticed — and `TestWriteEmbeddedCosignKey` that it is written out verbatim; the verifier refusal/opt-out invariant (`cosign_verifier_test.go`) is unchanged, driven through the `fetchKey` seam. Documented at `STRATA.md`. T4 stays `REFUTED` on #63 (`ec2runner` fetches the cosign *binary* with no checksum — a distinct trust-anchor gap) |
...
| B2, B3 | Recipes fetch sources with no digest pinning; the recipe schema has no field for one | A3 | #68 | Yes — **2026-09-16 (v0.27)**. Schema + adoption both landed. `RecipeMeta.Sources` carries `{url, sha256, arch}` per source, `Validate` requires each be pinned, and the build stages + verifies every source into `$STRATA_SOURCES` before `build.sh` (`internal/build/source.go`). **All 32 recipes** now pin their sources and read the verified artifact — arch-independent source tarballs (one digest), per-arch binaries (nodejs, julia, miniforge, quarto; rust rewritten from `rustup \| sh` to the pinned standalone toolchain), gcc (source pinned; `contrib/download_prerequisites` verifies GMP/MPFR/MPC against the pinned source's own SHA512), and CUDA (per-arch `.run`). `knownUnpinnedRecipes` is empty, so `TestRecipeSourcePinning_Guard` (`cmd/strata`) now asserts every fetching recipe pins. **Re-derived** at E1: all 31 buildable recipes built + signed + pushed for **both x86_64 and arm64** (62 layers) from their pinned sources on real EC2 (#229); staging/verification unit-tested in `internal/build/source_test.go`. glibc is pinned but a documented build stub (bwrap pending), so unbuilt. |
| B2, B3 | The build environment itself — `dnf` "Development Tools" and AMI system packages — is consumed by every build but not pinned or named in the recipe | A3 | #234 | Yes — **2026-09-17 (#234, v0.29)**. The build environment is now named and pinned. `LayerManifest.BuildEnvironment` (`spec/buildenv.go`) records the base AMI, the dnf `releasever`, and the exact NVR of every OS package a build installed; `strata build` captures it on the instance (`internal/build/buildenv_capture.go`) after the toolchain installs, and the build user-data pins `dnf --releasever` to the AMI's own AL2023 snapshot (read from dnf's own conf, exported as `STRATA_RELEASEVER`) so the toolchain resolves deterministically instead of tracking the moving repos. All 66 production layers (`s3://strata-registry`, 33 recipes × 2 arches) were rebuilt against fresh per-arch gcc and carry a populated `BuildEnvironment` — verified x86 `ami-0c421724a94bba6d6`/`2023.10.20260302`, arm64 `ami-02c13950b5ec1f04e`/`2023.12.20260914`. **Re-derived** at E1 (chosen/implementation): `spec/buildenv_test.go` (sort/dedup determinism, `Recorded()` truth table, manifest round-trip) and `internal/build/buildenv_capture_test.go` (complete record on an rpm host, nil off one). **B2 (input completeness) is discharged**: every build input is named. **B3 is deterministic up to the AL2023 versioned repo and the fixed AMI** — the recorded NVRs plus the `releasever` pin make a re-resolution reproducible — but binary bit-for-bit reproducibility, and moving the toolchain into attested core layers rather than `dnf` (Option B), remain future work. With #68 (sources, v0.27) and #234 (build env) both discharged and no other live counterexample, B2/B3 leave `REFUTED`. |
...
| T3 | Stage 7 holds a bundle URI, not bundle bytes, so it cannot Rekor-verify | A4 | #85 | Yes — **decided 2026-09-14 (#85)**. The row recorded stage 7 as a place that ought to Rekor-verify but cannot; the decision, now made and recorded at the call site (`internal/resolver/stages.go`), is that it is *not* a verification point. Resolve-time stage 7 does presence-and-structure only — bundles are URIs, not bytes — and cryptographic verification, which is where T3 applies, happens at the boundaries that hold the bytes: `strata verify --rekor` (T3-discharged by #59, the row above) and `strata run`'s pre-mount check. Stage 7 therefore never accepts a log entry as evidence for a layer — the branch is a fail-closed guard, and since #59 a real client given the only thing stage 7 can offer (a nil bundle) returns `trust.ErrNoBundle` rather than passing on index existence (A4). **Re-derived** at E1 (chosen/implementation): `internal/resolver/resolver_test.go` `TestStage7_RekorVerification` now records that stage 7 passes a nil bundle at index 42 and that a rejecting client propagates `REKOR_VERIFICATION_FAILED` naming the layer — so adding a bundle fetch here breaks that test rather than silently turning stage 7 into an unverified acceptance point (#86). T3 stays `REFUTED` on #88 (the `hashedrekord` body-shape tautology) |
| T3 | `hashedrekord` body shape inferred from our own `Log` — writer and verifier can be wrong together | A4 | #88 | No |
| R3 | A null entry in a software list is silently dropped and the profile resolves without it | H1 | #79 | Yes — **#79 fixed**. `Profile.UnmarshalYAML` (`spec/profile.go`) inspects the raw `software` sequence and refuses a null entry (`!!null`) before yaml.v3 can drop it — the drop happens at the sequence decoder, before `SoftwareRef.UnmarshalYAML` would run, so the check is on the document node. **Re-derived** at E1 (chosen/implementation): `spec/profile_null_test.go`'s `TestProfile_NullSoftwareEntryRejected` covers bare-dash, leading-null, explicit `null` and `~`, and the #79 pin in `spec/softwareref_yaml_test.go` is inverted to assert the refusal. R3 stays refuted by #102 (`Profile.Instance`/`Storage` parsed and dropped) |
| R2, R7 | `EnvironmentID` depends on YAML slice order when two layers share a `MountOrder` | H1 | #95 | Yes — **2026-09-14**, both halves closed at E1 (chosen/implementation). **Resolver half (R2):** `stage6TopoSort` tie-breaks the ready set by `lessByLayerContent` (name, version, ID) rather than `sort.Ints(queue)` (input index), so a resolved lockfile's `MountOrder` — and the `EnvironmentID` derived from it — does not depend on `software:` order. The by-name check `internal/resolver/mountorder_permutation_test.go` was red under the old `sort.Ints(queue)`; the full-serialisation instrument `internal/resolver/resolve_determinism_test.go:125` now marshals the whole lockfile forward and reversed and compares with the input/clock fields elided, mutation-checked (reordering by input index reddens it; reversing the content order does not). **`MountOrder`-tie half (R7):** `computeEnvironmentID` still falls back to slice order when two layers share a `MountOrder` (`spec/lockfile_hash.go:107-113`), but a tied lockfile can no longer reach a trust decision — `LockFile.Validate()` (`spec/lockfile.go`, `spec/lockfile_validate_test.go`, #65/#96) rejects a duplicate `mount_order` at every boundary — the CLI (run/publish/freeze/update), the EC2 agent (`Agent.Run`, added post-v0.25.0 after review found the agent unguarded), and `pkg/strata` — so the slice-order-dependent identity is never assembled or published. That removes the tie from R7's domain rather than making `computeEnvironmentID` itself order-total; the three coupled sort sites are left untouched because the boundary rejection makes the hash tie moot. **`ProfileSHA256` "any field":** this was R2's own too-strong wording, not a code defect — R2 is narrowed above to elide `profile_sha256` (and the two wall-clock fields) as input-recording, exactly as R1 elides `resolved_at`. **`Packages`-order clause was removed** (2026-09-13) as a misclassification: package order is content — the installer runs one command per entry in order (`spec/lockfile_hash.go:75-79`, `TestEnvironmentID_PackageOrderIsContent`). With the #95 counterexample discharged, R2 would derive `ENFORCED E1` — but the post-release adversarial review found a second, independent counterexample (overlapping formations, #208, the row below), so R2 stays `REFUTED`; this row is discharged on its own terms (standalone permutation and the hand-built tie). R7 stays `REFUTED` on #69 and #98, which this row does not touch |
| R2 | overlapping formations mount a shared layer twice; permuting them reorders `satisfied_by`/`from_formation` | H1 | #208 | Yes — **2026-09-14 (v0.26)**. `dedupLayers` (`internal/resolver/stages.go`) collapses layers with the same manifest ID into one before stages 4–8 — a layer from one registry resolves to one manifest per (name, version, arch, abi), so equal IDs are equal content — and merges provenance as the sorted union (`mergeSorted`), so the surviving layer records every requester independently of `software:` order. Two formations sharing `python` now mount it **once** with `from_formation` = `aformation@1.0, bformation@1.0` regardless of input order. The conflict check's same-formation exemption became set intersection (`sharesFormation`) rather than string equality, since a deduped layer can belong to several formations. **Re-derived** at E1 (chosen/implementation): `internal/resolver/overlapping_formations_test.go` (formerly the exclusion control) now asserts one instance, unioned provenance, and full-serialisation permutation-invariance. With both #95 and #208 discharged, R2 derives `ENFORCED E1` over standalone layers and overlapping formations both |
| R3 | the #208 dedup collapsed layers by unauthenticated manifest ID, so under A1 two same-ID layers with different content merged, dropping one requested, separately-signed layer from the set the signer then signs | A1 | #219 | Yes — **2026-09-15**, found in the v0.26 re-review. The manifest ID is not authenticated (only the squashfs digest and its cosign signature are, #146), so `dedupLayers` (`internal/resolver/stages.go`) now collapses two layers only when their SHA256 also matches and refuses an ID collision with differing content (`LAYER_ID_COLLISION`) — a real duplicate has an equal digest; a collision is a registry-integrity error, not a duplicate, and must not silently drop a requested layer (R3: a dropped request returned as a complete lockfile). **Re-derived** at E1 (chosen/implementation): `internal/resolver/dedup_collision_test.go` — two requested layers with different names and digests but one ID make resolution fail, and a genuine same-content duplicate still collapses (control) |
| I3, P1 | `IsFrozen()`/`EnvironmentID()` accept any non-empty string as a digest | A5 | #96 | Yes — **`LockFile.Validate()` 2026-09-14 (#65/#96)**. `IsFrozen()`/`EnvironmentID()` are unchanged — presence predicates by design — but a malformed digest can no longer reach a trust decision: `LockFile.Validate()` (`spec/lockfile.go`) rejects a non-empty layer `SHA256` or `Base.AMISHA256` that is not 64 lowercase hex (via `ValidateLayerDigest`), a malformed layer id, and a duplicate `mount_order`; and every trust boundary calls it before acting — the CLI (`strata run`, which also stopped bypassing the parser #65; `publish`; `freeze`; `update`), the **EC2 agent** (`internal/agent.Agent.Run`, immediately after `Acquire`), and the public library (`pkg/strata.UploadLockfile`, `LockfileUserData`). **The agent and library boundaries were omitted at v0.25.0** — the release enumerated only the four CLI sites, and the post-release adversarial review found that `Agent.Run` acted on an unvalidated lockfile (a duplicate `mount_order` could reach the mounter and `EnvironmentID`); closed 2026-09-14, with `internal/agent/agent_validate_test.go` asserting a duplicate `mount_order`, malformed base digest, or unsafe layer id stops the boot before any fetch/verify/mount. Validation sits at the policy layer, not inside `ParseLockFileBytes`, so a diagnostic like `strata diff` can still read a malformed lockfile. **Re-derived** at E1 (chosen/implementation): `spec/lockfile_validate_test.go` rejects `bbbbbb`, `sha256-ami-test123456789`, an uppercase digest, a path-traversal id and a duplicate mount order, and accepts a well-formed frozen and an unfrozen lockfile. I3's other refutation, #81 (the strata-agent fetcher), is discharged 2026-09-14 — the fetcher now validates via `LayerCachePath` — so I3 leaves `REFUTED`; P1 stays refuted by #66 |
| I4 | `internal/overlay/mount_linux.go:144` and `internal/export/oci.go:63` build paths from an unvalidated `layer.ID` | A1 | #97 | No |
| I6, P3, X1, X2, R7 | `packages:` installs ignore the recorded SHA256; conda `latest` resolves at boot — so `Packages` participate in the identity and still fail to determine the environment. The row carries **both** directions and they are worth separating, because until 2026-08-21 (§7) it named only the second: an install that ignores the recorded `sha256` means two lockfiles differing only in that digest fetch identical bytes, so the environment is the same and the identity is not — that is **R7**. Conda `latest` resolving at boot is the converse, and that is X2 | A3, A7, H1 | #98 | Partially — **2026-09-16 (v0.27)**: the installer now reads the recorded digest instead of ignoring it. pip enforces it — `pip install --no-deps --require-hashes name==version --hash=sha256:<recorded>` when a SHA256 is present (`internal/agent/package_installer.go`), so two lockfiles differing only in a pip digest install different bytes or fail (the **R7** direction), and conda refuses a `latest` or empty version rather than resolving it at boot (the **X2** direction). **Re-derived** at E1: `internal/agent/package_installer_test.go` (hash enforced when present; version-pinned but no `--hash` when absent; floating pip and conda versions refused with nothing installed). Still live: a pip entry with an **empty** recorded SHA256 installs the pinned version but unverified bytes, so byte-reproducibility depends on the resolver populating every digest (and conda has no per-package hash flag) — and #139 is the operator-facing disclosure of the same gap. |
| P2 | The Zenodo deposit contains the lockfile only — no bundles, no key, no layers | H1 | #99 | Partially — **2026-09-15 (v0.26)**. The deposit is now a verification bundle: `internal/zenodo/zenodo.go` `Deposit` uploads the cosign public key (`cosign.pub`, the embedded trust anchor #62/#222) and a `STRATA-VERIFY.md` manifest alongside the lockfile, whose inline `bundle` field is the signature over the whole set — so the environment's attestation is verifiable from the record alone, and the digests are no longer unfalsifiable. **Re-derived** at E1: `internal/zenodo/zenodo_bundle_test.go` (the key and manifest are deposited; the key is the embedded anchor byte-for-byte). Still live: per position (a) of #99, layer *bytes* and per-layer bundles are not deposited (too large), so byte-level confirmation is not registry-independent — it fetches from the registry with the signed digest as the binding. |
| T6 | Nothing records the trust policy a result was produced under | H1 | #100 | Yes — **2026-09-14**. `LockFile.VerificationPolicy` (`spec/lockfile.go`) records what the resolver checked — `attestation-references-present` when every layer named a bundle and Rekor entry (and stage 7 refused any that did not), `unsigned-offline` when the offline path accepted an unsigned layer — so a consumer reading only the lockfile can tell what was checked. The label was renamed from `attestation-present` (post-release review) to say *references*, not verified attestation: a resolve holds URIs, not bundle bytes (#85). Set by the resolver (`internal/resolver/stages.go`, `verificationPolicy`) and kept out of `EnvironmentID` on the `inert` route (asserted by `TestEnvironmentIDRoutes_MatchBehaviour`, `TestEnvironmentIDRoutes_CoverEveryField`). **Re-derived** at E1 (chosen/implementation): `internal/resolver/verification_policy_test.go` for the label, `TestStage7_BundleMissing`/`TestStage7_RekorEntryMissing` for the enforcement that makes it more than a value computed from the fields it names. It records the resolve-time policy, which per #85 is the strongest a resolve can honestly claim; a lockfile-level *verification* result (verify/run against the log) is #60/#101 territory and does not weaken this |
| T8 | rollback/freeze: a stale but previously-valid, correctly-signed lockfile or layer replays undetected | A6 | #224 | Partially — **2026-09-16 (v0.28.0, #224)**. `resolved_at` is inside the signed payload, so a freshness measure is tamper-resistant once the set signature verifies. Freshness is now *surfaced always and refusable opt-in*: `LockFile.Age`/`FreshnessNote`/`CheckFreshness` (`spec/freshness.go`) compute the staleness and refuse a lockfile past a caller-set bound, wired at every consumption boundary **after** signature verification — `strata run --max-age` (`cmd/strata/run.go`, surfaces the age and refuses past the bound), `strata verify --max-age` (`freshnessFailures`, so a stale lockfile no longer passes silently — a diagnostic's green was the danger), and the EC2 agent (`internal/agent/agent.go`, from `STRATA_AGENT_MAX_AGE`), which refuses to boot a stale lockfile before any fetch/verify/mount. **Re-derived** at E1 (chosen/implementation): `spec/freshness_test.go` (age/bound/parse, incl. the exactly-at-bound boundary and a no-`resolved_at` refusal), `internal/agent/agent_freshness_test.go` (a bound refuses a stale boot before fetch/verify/mount; with no bound a stale lockfile still boots and its age is surfaced), `cmd/strata/verify_freshness_test.go`, `cmd/strata-agent/max_age_test.go`. **Stays REFUTED (partial):** the refusal is opt-in by design — a default max-age would refuse a frozen, cited, archived lockfile, which is exactly what strata exists to re-run — so a stale signed lockfile is *detectable* everywhere and *refused* only where a bound is set, and the fuller TUF-style signed-timestamp/snapshot role (an authenticated "this set is current" assertion independent of the artifact's own `resolved_at`) remains future work. Split from #101 to #224. |
| X1, R3 | `Profile.Instance` and `Profile.Storage` are parsed and referenced nowhere else | H1 | #102 | Yes — **documented advisory + tested 2026-09-14**. Both fields now carry doc comments in `spec/profile.go` stating they record intent for downstream launch tooling and are not consumed by resolution — the same honest exemption `RequiresHost` takes, which X1 names as its rule applied rather than violated: a *stated* unacted field is not the silent one X1 forbids. **Re-derived** at chosen/implementation: `internal/resolver/instance_storage_advisory_test.go` resolves a profile with and without them and asserts identical identity and layers, so they reach neither the lockfile (which carries neither field) nor the resolved environment. This discharges the X1 instance (stated, not silent) and the R3 instance (their absence from the lockfile is by design, not a partial resolution). Wiring them into an actual launch/mount path is separate future feature work |
| R7 | `EnvironmentID` distinguishes a package set whose inner `Packages` slice is `nil` from one where it is empty. A set with no entries installs nothing either way, so the environment is identical. `spec/packages.go:52` carries `json:"packages"` with no `omitempty` while the **outer** `LockFile.Packages` field has it, so the inner slice marshals as `null` in one case and `[]` in the other and `computeEnvironmentID` hashes different bytes. Measured on `048aea4`: `59cc9349bf262f0ac84ae8ec74b3c768af016efe128dc66ae3a93179ffbdd9e4` versus `57620b0c2fa034e49fd39f507a180f28d42819a3996f7300a91d0a0d7b6c5076`. Reachable from ordinary input, not only hand-built structs: YAML `packages: []` decodes to an empty non-nil slice while an omitted key leaves it nil. Distinct from #95, which is about order rather than emptiness | H1 | #117 | Yes — #145, merged `69f789d`. **Re-derived 2026-09-13** (§2.1 rule 11): `packageSetHashInput.Packages` now carries `omitempty` (`spec/lockfile_hash.go:83`), so a nil and an empty inner slice hash identically and the two measured IDs (`59cc9349…`/`57620b0c…`) agree. Standing evidence at E1 (chosen/implementation): `TestEnvironmentID_NilAndEmptyInnerPackagesAgree` (`spec/environment_id_scope_test.go:797`), green at `a1e557b`. The fix is in the hash projection, not on the wire struct — `spec/packages.go:52` keeps `json:"packages"`, which is content read in order |
| X2 | `LockFile.Defaults` alters the assembled environment and does not participate in the identity, so two lockfiles that assemble **materially different** environments share one `EnvironmentID`. `Defaults` is declared `spec/lockfile.go:45-47`, copied `internal/resolver/stages.go:438`, and consumed at `internal/overlay/overlay.go:167-185`, which writes `module load <name>/<version>` lines into `/etc/profile.d/strata-defaults.sh` inside the assembled root — so it decides which module versions are active in every login shell. `envHashInput` (`spec/lockfile_hash.go:19-25`) has five members and `Defaults` is not one. Measured on `048aea4`: three lockfiles identical but for `Defaults` — none, `python/3.11.9`, `python/3.9.18` — all hash to `09f451dc302d078526dfcd8f1d50ab245ab1ba6f888f9693933db687c7f2c339`. This is the **converse** direction to #95/#69/#98/#117: same ID, different environment, so anything keying a cache or an attestation on `EnvironmentID` can vouch for the wrong environment. R7's generator cannot find this class — it asserts equal-environment-implies-equal-ID, and this is the other implication | H1 | #118 | Yes — #145, merged `69f789d`. **Re-derived 2026-09-13** (§2.1 rule 11): `Defaults` is now hashed as `moduleHashInput{Name,Version}` (`spec/lockfile_hash.go:40,65-68,126-129,151`), so the three-lockfiles-one-`09f451dc…` measurement no longer reproduces. Standing evidence at E1 (chosen/implementation): `TestEnvironmentIDRoutes_MatchBehaviour/LockFile/Defaults` (`spec/environment_id_scope_test.go:633`) asserts the route is `content` and that mutating `Defaults` moves the identity, and `TestEnvironmentIDRoutes_CoverEveryField` (`:590`) fails the build if any field loses its route. Green at `a1e557b` |
| X2 | `ProfileName` and `RekorEntry` alter the assembled environment at five sites and do not participate in the identity — and unlike `Defaults`, the exclusion is *argued for* in the source: `spec/lockfile_hash.go:14-16` says attestation and identity fields "do not affect what runs". They affect what runs at `internal/overlay/overlay.go:145-146` (`export STRATA_PROFILE`, `export STRATA_REKOR_ENTRY` in `/etc/profile.d/strata.sh`), `:197-198` (the same two in `/etc/strata/environment`, a systemd `EnvironmentFile`), `cmd/strata/run.go:373,375` (the child process environment), `internal/fold/eject.go:196-197,213-214` (the ejected artifact), and `internal/export/oci.go:393,406-407` (OCI image `Env` and labels). Measured on `93df7ca`: two frozen lockfiles differing only in these two fields both return `6680a2f4279081f86b759e179ae35a873c47730e352c61f23b6427c7317d35d1` while `/etc/profile.d/strata.sh` and `/etc/strata/environment` differ. The false comment is the direct cause of two unsound justifications in the R7 generator, recorded in §7 | H1 | #120 | Yes — two halves, two reasons. **`ProfileName`: #145, merged `69f789d`** — now hashed (`spec/lockfile_hash.go:37,148`); `TestEnvironmentIDRoutes_MatchBehaviour/LockFile/ProfileName` asserts the identity moves. **`RekorEntry`: #121** — `STRATA_REKOR_ENTRY` and `LockFile.RekorEntry` take the **declared-provenance** route (`spec/lockfile_hash.go:27-30`), which §3's scope clause excludes from "the assembled environment" by name; the absence of an in-tree reader is *enforced* by `TestDeclaredProvenance_NoInTreeReader`, not asserted, and the `RekorEntry` row of `MatchBehaviour` confirms the identity does not move. **Re-derived 2026-09-13** at E1 (chosen/implementation), both green at `a1e557b`. The false comment at the old `spec/lockfile_hash.go:14-16` is corrected in place, which also retires the two unsound R7-generator justifications |
| X2 | `PATH` and `LD_LIBRARY_PATH` — the entire mechanism by which a Strata environment is the environment it claims to be — are built from three `LayerManifest` fields outside the identity. `internal/overlay/overlay.go:111-130` reads `InstallLayout`, `Name` and `Version`; `envHashInput` holds only `LayerSHA256s`. `InstallLayout` decides whether a layer contributes to `PATH` **at all** (`if layer.InstallLayout == "flat" { continue }`), so one identity covers both "this layer is on `PATH`" and "this layer is absent from it". Measured on `93df7ca`: three pairs identical in every hashed field all share `a5f6e562af5a9a1999231f6c6d0c3b250598f3f1ec425ec163272976ba961edf`, and the `flat`-versus-empty pair produces one root with no `PATH` line and one with `/strata/env/python/3.11.9/bin`. The `Name`/`Version` pairs are additionally *broken*, since the mount path comes from the manifest while the directory structure lives inside the squashfs | H1, A1 | #122 | Yes — #145, merged `69f789d`. **Re-derived 2026-09-13** (§2.1 rule 11): `Name`, `Version` and (canonicalised) `InstallLayout` are now hashed per layer (`layerHashInput`, `spec/lockfile_hash.go:52-58,117-122`), so the three pairs no longer share `a5f6e562…`. Standing evidence at E1 (chosen/implementation): `TestEnvironmentIDRoutes_MatchBehaviour` over `LayerManifest/{Name,Version,InstallLayout}` (`spec/environment_id_scope_test.go:633`), and `TestEnvironmentID_EmptyInstallLayoutEqualsVersioned` (`:771`) pins that only the *canonical* empty-equals-versioned identity holds — an empty layout and `"versioned"` are the same environment and must not fork the ID. Green at `a1e557b` |
| P3 | `EnvironmentID` is a storage key, so a collision is destructive rather than merely ambiguous. `internal/registry/s3client.go:620` and `localclient.go:374` write `locks/<environmentID>.yaml` with no conditional put and no existence check, and `PutLockfile`'s godoc directs the caller to tag an instance with the returned URI so `strata-agent` fetches it at boot. Measured on `93df7ca` against `LocalClient`: two frozen lockfiles differing only in `Defaults` both return `a5f6e562…`, `locks/` holds **one** file after both puts, and the stored `Defaults` is the second writer's — so which environment an instance boots depends on publish order, not on the identity it was launched with. Same function: `EnvironmentID()` returns `""` for an unfrozen lockfile and `pkg/strata.Client.UploadLockfile` has no `IsFrozen` guard where `cmd/strata/publish.go:76` does, so an unfrozen lockfile is accepted and written to `locks/.yaml`. Both premises were pinned executably against `LocalClient` by `internal/registry/lockfile_key_test.go`, whose own comment instructed its deletion once #124 was fixed | H1, A1 | #124 | Partially — **#174, merged `a1e557b`**, closes the counterexample at the publish boundary. Both `PutLockfile` implementations now refuse an empty `EnvironmentID()` (no more `locks/.yaml`) and write conditionally so a colliding key is a loud error, not a silent overwrite: `internal/registry/localclient.go` opens with an exclusive `O_EXCL` create, `s3client.go` sends `IfNoneMatch:"*"`. **Re-derived 2026-09-13** at E1 (chosen/implementation): `internal/registry/putlockfile_conditional_test.go` refuses the empty id on *both* backends and refuses an `O_EXCL` collision on the local backend, and replaced the deleted defect-pin. **What remains is the *S3* half, still held by reading (rule 4, rule 6):** the package's `mockS3` ignores `IfNoneMatch`, so the S3 conditional cannot be executed without a live `s3client` run; the local `O_EXCL` guard exercises the equivalent mechanism, read-matched against `s3client.go`. `Partially` counts as live, so P3 stays refuted — and via #98 regardless. The guard's strength against a *different environment at the same id* is further bounded by digest-format validation (#96/#65) |
...
- #84 — `internal/registry FetchLayerSqfs` has no test that can observe the #57
  ...
  third sites, not I2.
- #86 — `TestStage7_RekorVerification` asserted a negative case its body never
  exercised. Rule 6. **Closed 2026-09-14:** the test now drives a rejecting
  client to `REKOR_VERIFICATION_FAILED` (naming the layer) and uses a recording
  client to pin what stage 7 passes — a nil bundle at the layer's log index —
  so the #85 fact is test-visible and a future bundle fetch breaks it.
...
### 6.2 Citations must resolve, and the convention that lets a machine check them

An evidence citation is a `` `path:line[s] SymbolName` `` reference, and a stale
one is a *valid wrong line*: the file exists and the line exists, so an
existence check reads it as green. `internal/propdoc.CheckCitations`
(`go run ./cmd/propgen -citations`, enforced by `TestPropertiesCitationsResolve`)
therefore checks three things for every **machine-resolvable** citation — a
repo-relative path resolves, every cited line is within the file, and where a
symbol is named it appears within the cited span — and, when the working tree
fails, retries against any commit id on the citation's line, so a pin to a past
commit resolves against the code as it was (a pinned id must be an ancestor of
`HEAD`).

Two obligations on whoever writes a citation:

- **Prefer the resolvable form: a repo-relative path, and the symbol named.** A
  basename-only `` `agent.go:299` `` or a bare `` `:299` `` cannot be located
  without a search and is not checked — 24 basename-only citations remain, tracked
  by #105 as the notation-normalisation half.
- **A discharged refutation's citation is pinned to the commit it was measured
  on** (rule 11's tense constraint): write the commit id on the same line, since
  renumbering it to where the code now lives would destroy the counterexample's
  reproducibility. Resolving it against the working tree is the wrong tense.
...
11. **Rule 7 added** — a false documented claim is a refutation, not E0. E0 as
    drafted rated `internal/trust/verify.go:93-94` the same as an honest
    silence.
...
32. This document and #104 touch no common line, so nothing will report a
    ...
    If #103 merges first, then on #104 merging: `internal/agent/agent.go:273` →
...
38. **A discharge was matched to a claim about a different registry, and the
    ...
    when `STRATA_REGISTRY_URL` is unset (`cmd/strata/resolve.go:108`), and its
    ...
    — which `cmd/strata/resolve.go:99-100` states in a comment, in the tree, the
...
39. **#108 added as a register row against X3 and P4** — none of the six shipped
    ...
    `validated_on: [al2023/x86_64, al2023/arm64]`, a field `spec/layer.go:226-228`
...
42. **Two register rows remain discharged on issue closure, and are filed rather
    ...
    that **is** present: `internal/resolver/stages.go:74-78` emits the unattested-
    formation warning (observed by command in item 38's transcript) and
    `cmd/strata/run.go:107` prints the `packages:` warning. Neither is asserted by
    any test —
...
47. **`validated_on` gets a register row against X1 (#113), split out of #108.** Five
    ...
    `spec/layer.go:228` and its doc comment at `:223`, and **nothing else** — no
    ...
    and never adds a layer to satisfy one; stage 4 (`internal/resolver/resolver.go:122`)
...
49. **The `pending-initial-build` placeholders get a register row against X1 (#46),
    ...
    (`internal/resolver/stages.go:74-78`, the deliberate outcome of #49) and never
...
52. **R1 restated: `TOO STRONG` → `SOUND`, and its status falls from `ENFORCED E1`
    ...
    *What the old statement permitted that this one forbids.* An unsatisfiable
    ...
    `internal/resolver/resolver_test.go:601 TestEnvironmentID_Stability` compares
...
53. **R2 restated: `TOO WEAK` → `SOUND`; it stays `REFUTED`.** The old form carried
    ...
    *What can now refute it that could not before.* A permutation generator over
    ...
    `sort.Ints(queue)` at `internal/resolver/stages.go:304` breaks `MountOrder` ties by
    slice position (#95).
...
54. **R6 withdrawn, not restated — the attempt to restate it is what retired it.**
    ...
    *Why withdrawn rather than narrowed*, which is the question the restatement
    ...
    `spec/lockfile_hash.go:19-25`. That form is sound, and falsifiable **only by drift
...
59. **Two counterexamples filed, and they run in opposite directions.**
    **#117** (R7): a package set's inner `Packages` slice hashes differently when
    `nil` than when empty, because `spec/packages.go:52` lacks the `omitempty` the
    ...
    `/etc/profile.d/strata-defaults.sh` (`internal/overlay/overlay.go:167-185`) and is
...
64. **Two of the generator's five transformations were unsound, and the unsoundness
    ...
    paraphrasing `spec/lockfile_hash.go:14-16`: *"they do not affect what runs"*.
...
66. **R7 is vacuous and X2 is trivially refuted, both for one reason, and neither
    ...
    `internal/overlay/overlay.go:210-214` marshals the whole lockfile into
    ...
    `cmd/strata/cache_prune.go:38` defaults `--lockfile` to `active.lock.yaml` and
...
67. **The identity is a storage key, which turns every collision in this class from
    ...
    it to `locks/.yaml`, where `cmd/strata/publish.go:76` guards exactly that case
...
78. **Rule 11 gets run against the rows that were already in the table, and two of the
    ...
    **#49 is `Partially`.** The warning exists, fires, and names the formation, the
    reason and the value (`internal/resolver/stages.go:74-78`), which is E1 at
    ...
    `pkg/strata/strata.go:106-110` passes nothing, with no `Options` field a caller could
    ...
    *"are unattested"*. The shipped warning (`cmd/strata/run.go:95-108`) does what #48
    ...
    (`internal/agent/package_installer.go:40-48`). So the row as written is not
    ...
    **T7's live count moves 1 → 3 of 4, and this is the failure mode the derived column
    ...
    (`internal/propdoc/propdoc.go:120`), so both cells counted as discharged and §3
...
79. **The mechanical half of rule 11 becomes a check, the unmechanical half gets a
    ...
    (`internal/propdoc/propdoc.go:453`) now reports every `Yes` row naming no basis or
    citing no artifact, `propgen` refuses before it rewrites the Status column
    (`cmd/propgen/main.go:54-60`), and `TestPropertiesRegisterMeetsRule11`
    ...
    **The distribution caveat is restated as a property of the generator.** This is the
    ...
    (`internal/propdoc/propdoc.go:342`) and `Kind` discards parenthesised detail by
...
80. **The Basis column was a max, and a max over unevenly covered routes overstates.**
    ...
    **The replacement is a meet, and the correction to the obvious form of it is the point.**
    ...
    (`internal/propdoc/basis.go:185`) and `parseBasis` (`internal/propdoc/propdoc.go:585`).
...
82. **R4 and R5 discharged — the same instrument that reproduced them is now the barrier.**
    Item 76 (#133) stood these two rows up as `REFUTED` with an executable reproduction where
    before they were `SOUND` on a Basis of `none`; #153 fixes the defect, and that same
    `provider_matrix_test.go` is now the regression barrier the discharge cites — reproduction
    and discharge are one test, not two. #153 collapsed
    stage 4's first-match `SatisfiesRequirement` and stage 6's last-provider-wins edge into
    one predicate, `spec.Capability.Satisfies` (`spec/layer.go:189`), and made the edge follow
    the highest satisfying version with a lowest-layer-ID tie-break. The §4 row naming both
    propositions moves to `Yes`, and `propgen -write` carries R4 from `REFUTED` to
    `ENFORCED E1` — `chosen/implementation` carries the legacy name E1 in §2's basis table — and R5 to
    `ENFORCED exhaustive/implementation`, the **first cell in the document to render a pair-basis
    `ENFORCED` with no legacy `E`-name**. That the two discharge to *different* spellings is item
    76's point realised: it filed R5 `exhaustive` and R4 `chosen` precisely so the difference the
    retired ladder hid — which showed both as `E1` — is visible here.

    **Re-derived, not closure-discharged (rule 11).** The `Yes` cites the three tests run green
    at the merge head `9e918de`, not the closed issue: R5's 12-of-72 rejections and R4's
    3-of-12 misorderings are both 0, and `TestStage6SelectsHighestSatisfyingProvider` — new in
    #153 — pins that the surviving edge follows version rather than slice order, a distinction
    `TestStage6ProviderSoundness` cannot draw because it carries a single satisfying provider.

    **The discharge changed which assertion is load-bearing for R5, and that is the finding.**
    A count of violations going to 0 is discharged by an implementation that *rejects nothing*
    as readily as by one that is correct — a zero-count assertion has a trivial model. So R5's
    basis now rests on the order-independence of stage 4's verdict, which has no fixed point,
    guarded by `wantDiscriminating=12` against the vacuous case where the two orders never
    disagree. The 12 is the same 12 the old violation count held; the property replaces the
    count rather than supplementing it.

    **Two stale Basis anchors corrected in the same edit — item 80's class again.** The rows
    cited `provider_matrix_test.go:239` (R4) and `:116` (R5); #153 grew that file from 341 to
    539 lines, so both were the *valid wrong line* an existence check reads as green — `:239`
    now lands mid-error-string, `:116` on `}}`. Both replacements name the symbol they anchor
    (`TestStage6ProviderSoundness`, `TestStage4ProviderCompleteness`), per #105, which is what
    makes them checkable rather than merely correct today. The stage/`layer.go` line ranges
    inside the Evidence prose describe the *pre-#153* code and are labelled as such rather than
    repointed, because they are the refutation record, not a claim about the current tree.

    **The apparatus/defect purpose counter advanced at #153, not here.** #153 was the
    shipped-code fix — `spec/layer.go`, `internal/resolver/stages.go`. This
    PROPERTIES.md change is apparatus that records the discharge, and a `Discharged` change
    travels alone, so it touches this file only. Re-derivable on `main`:

    ```sh
    go test ./internal/resolver/ -run 'TestStage4ProviderCompleteness|TestStage6ProviderSoundness|TestStage6SelectsHighestSatisfyingProvider' -count=1
    go run ./cmd/propgen   # exit 0: the Status column already agrees with the moved register row
    ```

    (2026-09-12, #67 / #153.)

### 2026-09-13 — the scope of "the assembled environment" is stated (#121)

83. **R7 and X2 quantify over "the assembled environment" and no section defined it,
    so R7's premise was vacuous — every lockfile field is written verbatim into
    `/etc/strata/active.lock.yaml` in the assembled root, under which no two distinct
    lockfiles assemble the same environment.** §3's preamble now defines the term:
    mounted layer content plus the exported process environment, **excluding the
    provenance record** (`active.lock.yaml` and `STRATA_REKOR_ENTRY`). That makes
    R7's domain non-empty and X2's refutations substantive rather than trivial; both
    stay `REFUTED` — their live counterexamples (R7: #69, #98; X2: #118, #122, and the
    `ProfileName` half of #120) are untouched by a scope clause — so `propgen` shows
    no drift. Only the `RekorEntry` half of #120 and the #123 doc become dischargeable
    on the strength of this clause, and those discharges travel in their own change.

    **The exclusion is per field, not per file, and that is the substance.**
    `active.lock.yaml` is not inert: `cmd/strata/cache_prune.go:38,59-62,120` reads
    `Layers[].SHA256` out of it and deletes every cached `.sqfs` not in that set. It is
    safe only because that one field is in `envHashInput`; a blanket "the provenance
    record is out of scope" would license a future reader of an *unhashed* field to
    drive that destructive action while the identity stayed blind to it. So the clause
    is written per-field: a value in `active.lock.yaml` is out of scope exactly while
    nothing outside the provenance record reads it.

    Also corrected the live citation for the whole-lockfile write from
    `internal/overlay/overlay.go:210-214` (the `PATH`/env-file write) to `:221-228` (the
    actual `yaml.Marshal(lockfile)` + `WriteFile`), in R7's evidence cell and §3. The
    dated entry above (item 66) keeps its original citation as the historical record.

    (2026-09-13, #121.)

### 2026-09-13 — the two core trust propositions are enforced, and a release-refresh gate (#172)

84. **T1 and T5 move `REFUTED` → `ENFORCED E1`; the release refreshes the register
    against the code that shipped in it.** #93 (the agent is fail-closed by default;
    `Config.AllowUnverified` is the explicit opt-out) shipped in v0.23.0, but the
    tagged `PROPERTIES.md` still described the nil-verifier path as open at
    `agent.go:285` — the register prose had outrun the code. The T1/T5 register row
    (#93 nil-verifier + #104 absent-bundle) is discharged on re-derived evidence:
    `internal/agent/agent.go:302-306` refuses a nil `Verifier`/`BundleFetcher` unless
    `Config.AllowUnverified` is set, and `internal/agent/boot_matrix_test.go`
    enumerates all 112 decision-surface cells with `knownOpenCells` now `0` — the 20
    cells that formerly reached the mount unverified assert the refusal, and the
    `knownOpen` branch went red once per cell when `Run` began refusing. Both halves
    of the row are closed, and it was the last live refutation of both T1 and T5, so
    `propgen` recomputes each to `ENFORCED E1 (weakest of N scopes)`. T1's verdict
    stays `TOO WEAK` — its *text* still admits a fail-open under an unstated policy —
    but under the strengthened reading (rule 9) its Status is now enforced. The
    distribution shifts `REFUTED 25 → 23` and `ENFORCED E1 3 → 5`, and this is the
    first time the document's own Status column renders a multi-scope basis reduction
    (§3.1 and `basis_scope_test.go` updated: the T1/T5 cells now display the reduction
    a live refutation previously outranked).

    **The process gap that let it ship stale is now a gate.** `propgen` regenerates
    the Status *column* from the register, but the register *prose* is hand-authored
    and lagged the code. A release-refresh step is added to the release process
    (CLAUDE.md §Versioning): before tagging, discharge the register rows for the
    issues the release closes on re-derived evidence, confirm `propgen` shows no
    drift, and confirm the §3 prose and distribution header match the RC — so the
    instrument that establishes trust is not temporally ambiguous at the tag.

    (2026-09-13, #172.)

### 2026-09-13 — citations are checked, and the checker found four stale ones (#105)

85. **Nothing bound a `path:line` citation to the code it points at, and the live
    defect class is a *valid wrong line* — the path and line both exist, so an
    existence check reads green.** `internal/propdoc.CheckCitations`
    (`go run ./cmd/propgen -citations`; enforced by `TestPropertiesCitationsResolve`)
    now resolves every machine-resolvable citation: the path exists, every cited
    line is within the file, and a named symbol appears within its cited span —
    retried against any commit id on the line so a pin to a past commit resolves
    against the code as it was, the id required to be an ancestor of `HEAD`. Run
    against the existing document before being called insurance (per the issue's
    own note that at least one anchor had been wrong since it was written), it found
    **four** stale citations and they are fixed here: `internal/agent/agent_test.go`
    citing `:181` for `TestRun_SHA256Mismatch` now at `:203`; two partial paths
    missing their `internal/` prefix (`overlay/mount_linux.go`, `export/oci.go`);
    and R5's pre-fix `SatisfiesRequirement` record in `spec/layer.go`, which the
    working tree no longer matches (the symbol moved to line 225 at #153), now
    pinned to `058e6e1`, #153's parent. A control (`TestCitationCheckCanFail`) corrupts
    citations of three kinds and requires exactly those defects, since a checker
    that resolves nothing looks identical to a clean document.

    **What is deliberately not built yet** (scoped to #105's follow-ups): the
    24 basename-only citations are counted, not checked, until the notation pass
    normalises them; and Level 2 — coverage of the cited implementation lines, the
    proxy for rule 6's *does this test exercise the property* — is a separate step.
    So this is existence + symbol-at-line, the drift guard, not yet the
    exercises-the-property guard. §6.2 records the convention.

    (2026-09-13, #105.)

### 2026-09-13 — the shipped catalog resolves offline, discharging #108 and #54

86. **None of the six shipped formations resolved against the embedded catalog —
    the on-ramp a fresh clone with no AWS actually hits — and no test noticed.**
    The catalog carries no Sigstore bundles and stage 7 refused every layer;
    `hpc-mpi@2026.03` failed even earlier at stage 4 for want of
    `ucx`/`hwloc`/`pmix`/`libfabric`. Fixed two ways (#108): stage 7 accepts an
    unsigned layer **only** on the no-registry offline path
    (`resolver.Config.AllowUnsignedOffline`, set by `cmd/strata/resolve.go`'s
    `offlineCatalogOnly`, warned once), leaving a real registry strict; and
    `hpc-mpi` gained `openmpi`'s runtime deps. `TestShippedFormationsResolveOffline`
    now resolves all six and is the test that was missing — with the flag forced
    off it fails all six, `hpc-mpi` now at stage 7 rather than stage 4.

    This discharges the #108 register row (X3, P4) and, on the same fix, #54 (P4) —
    whose counterexample "stage 7 refuses every profile offline against the shipped
    catalog" no longer reproduces, evidenced this time against the **shipped
    catalog** rather than the fixture registry, closing the referent slippage that
    reopened #54 (item 38). Neither proposition leaves `REFUTED`: **X3** stays live
    on #70 (`examples/*` naming versions the catalog lacks) and **P4** on #64 (the
    offline lockfile is produced but unsigned and unfreezable — an offline resolve
    is explicitly not a trust decision). `X3` `2 of 3 → 1 of 3`, `P4` `3 of 3 →
    1 of 3`. The `validated_on` claim on the formations remains #113's concern; #108
    removed the *structural* impossibility (`hpc-mpi` could never pass stage 4), not
    the question of whether any of them was smoke-tested.

    (2026-09-13, #108.)

### 2026-09-14 — determinism gets its instrument, and it found a second wall-clock field (#95)

87. **R1 and R2 move to `ENFORCED E1`; both were awaiting one test, and writing
    it corrected R1's enumeration.** R1 (identical inputs produce byte-identical
    lockfiles bar the elided fields) and R2 (permuting `software:` changes no
    environment field) each named, in their own register rows, the instrument
    that could refute them — a differential resolve comparing the *full*
    serialisation with the input/clock fields elided — and neither had it. The
    standing tests compared derived hashes (`EnvironmentID`, `ProfileSHA256`) or,
    for R2, only `mount_order` by name, leaving every other field free to differ
    unobserved. `internal/resolver/resolve_determinism_test.go` supplies it:
    resolve twice for R1, forward-and-reversed for R2, marshal the whole lockfile,
    elide, compare — with a control asserting the two differ *before* eliding, so
    the elision cannot pass vacuously, and a mutation check that ordering the topo
    ready-set by input index reddens R2 while reversing the *content* order does
    not (a reversed total order is still permutation-invariant).

    **Writing the test found R1's elided set incomplete.** R1 enumerated exactly
    one field, `resolved_at`. The differential resolve reddened on a second:
    `base.capabilities.probed_at`, a clock reading the probe records and that
    `BaseCapabilities.ContentDigest` already excludes from the base identity
    (`spec/base_digest.go:15`) for the same reason `EnvironmentID` excludes
    `resolved_at`. It is now enumerated in R1 and R2. This is the differential
    instrument earning its place on input one: the register had *asserted* R1's
    elided set was complete, and it was not.

    **R2 is narrowed, along R1's line.** R2 read "does not change the resulting
    lockfile in **any** field", which `profile_sha256` refutes — it hashes the
    profile bytes, so reordering `software:` changes it. That is not an
    environment field; it records the input, exactly as `resolved_at` records the
    clock. R2 now elides `profile_sha256` and the two wall-clock fields, all
    enumerated in the proposition and not deferred to the specification — the same
    discipline the 2026-08-21 restatement imposed, applied to the input-recording
    fields the earlier pass left in scope. The environment invariant it exists to
    guarantee — `mount_order`, layer content, `environment_id` — is unchanged and
    now fully instrumented.

    **This also discharges #95.** Its two live halves are closed: the R2
    permutation half by the resolver's content tie-break (#176) plus the new
    instrument, and the R7 `MountOrder`-tie half by `LockFile.Validate()`
    (#65/#96), which rejects a tied `mount_order` at every trust boundary so the
    slice-order-dependent identity is never assembled — removing the tie from
    R7's domain rather than making `computeEnvironmentID` order-total. R7 stays
    `REFUTED`, now `2 of 4 live` (#69, #98), and the distribution moves
    `ENFORCED E1` 6 → 8, `REFUTED` 21 → 20, `UNPOPULATED` 3 → 2 — a rise in
    `ENFORCED` that is propositions catching up to evidence, the converse of the
    2026-08-21 fall and the same demonstration that the totals are not a
    direction of travel.

    (2026-09-14, #95.)

### 2026-09-14 — the agent fetcher validates its digest, and two integrity propositions enforce (#81)

88. **I3 and I5 move to `ENFORCED E1`.** The strata-agent layer fetcher built its
    cache path as `filepath.Join(cacheDir, layer.SHA256+".sqfs")` from an
    unvalidated digest — the one cache site #57 had not reached (it fixed the
    `strata run` side). A malformed digest was used as a filesystem path (I3), a
    `../` digest could name a file outside the cache (I4), and an empty digest
    collided with every other empty one on `.sqfs` (I5). `s3LayerFetcher.Fetch`
    now routes through `spec.LayerCachePath`, which validates 64-lowercase-hex
    before joining — the same function whose doc says every cache-path site must
    go through it, and this was the site that did not.

    The evidence is a fetcher whose S3 mock **holds** the object, so a fetch would
    succeed without the check: `TestFetch_RejectsInvalidDigest` asserts a
    too-short digest and a `../` traversal are refused anyway, which pins the
    rejection to the digest validation rather than a missing object. The inherited
    `TestFetch_CacheHit`/`CacheMiss`, which asserted Fetch *succeeds* with
    `abc123`/`deadbeef`, were inverted to 64-hex under the user-authorized
    inherited-test exemption for #81.

    #81 was I3's and I5's only live refutation, so both leave `REFUTED`; I4 keeps
    two live rows (#58, #97) and stays `REFUTED (2 of 4 live)`. Distribution moves
    `ENFORCED E1` 8 → 10, `REFUTED` 20 → 18. With #96 (I3's other row) already
    discharged by `LockFile.Validate()`, the digest-well-formedness proposition is
    now enforced at both the lockfile policy boundary and the agent's cache path.

    (2026-09-14, #81.)

### 2026-09-14 — the lockfile records what was checked, and T6 stops being ill-formed (#100)

89. **T6 is restated from `ILL-FORMED` to `SOUND` and enforced.** Its draft —
    "the trust policy … is recorded alongside that result" — named neither an
    artifact nor a lifetime, so a line on stderr satisfied it and no durable
    record also satisfied it; it could not be failed. The restated form names the
    artifact and the reader: *a lockfile carries a field naming the trust policy,
    and a consumer reading only the artifact can determine it.* Under that form it
    was refuted by absence — `grep 'TrustPolicy\|VerifiedAt'` found nothing.

    `LockFile.VerificationPolicy` closes it: the resolver records
    `attestation-references-present` (every layer named a bundle and Rekor entry,
    and stage 7 refused any that did not) or `unsigned-offline` (the offline path
    accepted an unsigned layer), so a reader of the lockfile can tell what was
    checked without knowing how the resolve was invoked. (The label was
    `attestation-present` at v0.25.0; renamed post-release so it cannot be read as
    "cryptographically verified" — it attests reference presence, since a resolve
    holds URIs, not bundle bytes.) It records the resolve-time policy only — a resolve holds
    bundle URIs, not bytes, so per the #85 decision it never claims log
    verification, and the two labels are what it can honestly say. This is the
    proposition #100 called "the only one in group T that would have made the
    other four fail-opens detectable after the fact rather than by code reading."

    The field takes the `inert` route out of the identity — recorded for audit,
    read by no assembler — so two lockfiles differing only in this label still
    share an `EnvironmentID`, asserted by `TestEnvironmentIDRoutes_MatchBehaviour`
    and required to have a route by `TestEnvironmentIDRoutes_CoverEveryField`.
    Distribution moves `ENFORCED E1` 10 → 11, `REFUTED` 18 → 17.

    (2026-09-14, #100.)

### 2026-09-14 — a post-release review found "every trust boundary" was four of six

90. **The #95/#96 discharges said `LockFile.Validate()` runs at every trust
    boundary; it ran at four of them, and the two it missed were the most
    important.** The enumeration was `run`, `publish`, `freeze`, `update` — all
    CLI. An adversarial review of the v0.25.0 tag found that `internal/agent.Agent.Run`
    — the EC2 boot path, the boundary that actually mounts layers and reports the
    `EnvironmentID` as an instance tag — acquired its lockfile through a raw
    `yaml.Unmarshal` and proceeded to fetch, verify, and mount with no structural
    validation. A lockfile with a duplicate `mount_order` and correctly-signed
    layers would pass every per-layer check and mount with YAML-order precedence,
    the exact case #95's boundary was supposed to make unreachable. The public
    library (`pkg/strata.UploadLockfile`, `LockfileUserData`) had the same gap and
    could feed such a lockfile to the agent.

    This is the failure the enumeration discipline exists to catch and did not: a
    primitive that works, a test for the primitive that works, four callers using
    it correctly, and the principal execution path omitted — recorded here because
    "every boundary" was prose, and prose is not a check. `Agent.Run` now validates
    immediately after `Acquire` (covering every present and future `LockfileSource`),
    and `UploadLockfile`/`LockfileUserData` validate before persisting or
    serialising. `internal/agent/agent_validate_test.go` asserts a duplicate
    `mount_order`, a malformed base digest, and an unsafe layer id each stop the
    boot before any fetch/verify/mount, with `SignalFailed` set — a negative boot
    test, so the boundary is enforced rather than enumerated. The #95, #96, and I3
    rows are corrected to name the agent and library boundaries and to record that
    they were absent at v0.25.0. No distribution change: the propositions were
    already `ENFORCED`/discharged on the CLI evidence; what moved is the honesty of
    the "every boundary" claim.

    (2026-09-14, review follow-up, #65/#96/#95.)

### 2026-09-14 — R2 back to REFUTED: the fixture was too thin, and the review found the gap

91. **R2 moves `ENFORCED E1` → `REFUTED (1 of 2 live)`; the code did not change,
    the fixture did.** The v0.25.0 discharge instrumented R2 with a
    differential resolve over three standalone, distinct layers, and PROPERTIES.md
    said so explicitly — "says nothing about profiles with formations or
    dependency edges." The post-release review executed exactly that untested
    domain: two formations sharing a layer resolve to a lockfile that mounts the
    shared layer **twice** and attaches `satisfied_by`/`from_formation` to the two
    instances in `software:` order. `EnvironmentID` is invariant (the hashed
    fields are the same multiset, which is why R7's generator cannot see it), but
    those two fields are not among R2's elisions, so the full lockfile is not
    permutation-invariant. `lessByLayerContent`'s final key is the layer ID, and
    two instances of one layer share it, so content cannot break the tie —
    confirming that R2's proposition ("name, then version, then digest") and the
    implementation (ID, which for a shared layer is identical) disagree exactly
    where a shared layer appears twice.

    Recorded rather than papered over: the fix is dedup-by-content with union
    provenance (#208), **not** eliding `satisfied_by`/`from_formation` from R2,
    which would hide the double-mount — the same layer in the stack twice is a
    correctness defect independent of the ordering. `overlapping_formations_test.go`
    is an exclusion control asserting today's double-mount and order-dependence, so
    #208's fix reddens it and forces R2's re-derivation. This is the enumeration
    lesson in the other direction: an `ENFORCED` that rested on a fixture too
    small to exercise the proposition's own stated domain, caught not by the
    generator but by a reader who built the case the register admitted it had not.

    (2026-09-14, review finding #2, #208.)

### 2026-09-14 — the shared layer is deduped, and R2 re-enforces over formations (#208, v0.26)

92. **R2 returns to `ENFORCED E1`, now over overlapping formations as well.** The
    refutation item 91 recorded had a root cause worth fixing rather than
    narrowing around: a layer requested through two formations was resolved twice,
    so the same squashfs was mounted twice and the two copies' provenance followed
    `software:` order. `dedupLayers` collapses layers with the same manifest ID
    into one before stages 4–8 and merges `satisfied_by`/`from_formation` as a
    sorted union, so the surviving layer records every requester independently of
    input order, and the shared layer is mounted once. The conflict check's
    same-formation exemption became set intersection (`sharesFormation`), because a
    deduped layer can belong to several formations — a one-element set reproduces
    the old string equality, so the existing conflict tests are unchanged.

    The exclusion control from item 91 was inverted rather than deleted: the same
    test now asserts one instance, unioned provenance, and full-serialisation
    permutation-invariance over overlapping formations — the domain the v0.25.0
    fixture could not reach. Deduping also corrects the identity: two identical
    layers hashed as two entries before, so `EnvironmentID` now reflects one
    python where the environment has one, which is R7's direction as well. With
    #95 and #208 both discharged and no live refutation, R2 derives `ENFORCED E1`;
    distribution `ENFORCED E1` 10 → 11, `REFUTED` 18 → 17.

    (2026-09-14, #208, v0.26.)

93. **A second adversarial trust-chain re-review: two code fixes and a live-row
    refresh.** The re-review of main scored the chain A but named four things.
    Two were code:
    - **#219 (dedup ID-collision, A1/R3).** `dedupLayers` (#208, item 92)
      collapses on `manifest.ID`, but the ID is not authenticated — only the
      squashfs digest and its cosign signature are (#146). So a tampering
      registry could stamp one ID on two separately requested, separately signed
      layers, and dedup would drop one requested, authenticated layer from the
      set the KMS key then signs (R3: a dropped request returned as a complete
      lockfile). Fix: collapse only when the SHA256 also matches; refuse an ID
      collision with differing content (`LAYER_ID_COLLISION`). This is why item
      92's dedup landed as a *content* dedup and not an *ID* dedup — the review
      forced the distinction the original phrasing blurred. New R3 §4 row,
      discharged at E1 on `internal/resolver/dedup_collision_test.go` (two
      requested layers, one ID, differing digests → refused; a genuine duplicate
      still collapses, control).
    - **#220 (rekor-entry binding).** `RekorEntry` is excluded from the signed
      payload — it cannot be signed before it exists — so altering only
      `lf.RekorEntry` left the set signature verifying, yet `ProvenanceRecord`
      publishes it as `lockfile_rekor_entry`, a pointer a paper or DOI may treat
      as authoritative. `VerifyLockFile` now binds it to the bundle's own log
      index and `SignLockFile` refuses a bundle with no tlog entry. New T3 §4
      row, discharged at E1 on
      `TestVerifyLockFile_RejectsRekorEntryMismatch`. Not a mix-and-match bypass
      — the genuine bundle still authenticates the set — but it stops a
      falsified provenance citation from shipping as "signature verified".

    The third was register currency: T2/T4/T9 and #101 still described the
    lockfile as having *no* signature verification. That was true before v0.26
    and false after (#60 landed a real `VerifyLockFile` on `verify`, `publish`
    and the agent; #62's key is embedded and verified against the KMS public
    half). Those rows now record that the set *is* signed and verified where
    verification runs, and stay **live** on the surviving gap: verification is
    **opt-in**, so an *unsigned* composed set is still accepted, and there is
    still no freshness bound (#101). No derived status changed — the rows were
    refuted before and remain refuted on the opt-in gap; this is a prose-accuracy
    fix, and the header now carries a `live rows current through` line so
    `measured at` (the last full manual re-audit, #104) is not misread as their
    currency. The fourth finding — mechanising the KMS↔embedded-key
    correspondence in release CI (LOW) — is tracked separately.

    (2026-09-15, #219 #220, v0.26.)

94. **Verification becomes mandatory, and `T9` moves to `ENFORCED E1`.** The
    v0.26 signing work landed a real `VerifyLockFile` but left it *opt-in*: every
    T-row that cited it (`T2`, `T7`, `T9`, and #60/#101) stayed live on the same
    ground — an *unsigned* or mix-and-match set was still accepted on the default
    path, because `strata run` verified each layer individually but never the set,
    `strata export` did no signature check at all (and raw-parsed the lockfile),
    and `strata fold` merged unverified layers into an artifact it then signed and
    pushed. #101 closes that: `run` verifies the set before mounting
    (`verifyRunSet`/`verifyRunLayers`, `cmd/strata/run.go`), `export` verifies
    before packaging, and `fold` verifies its inputs before signing — the last two
    against the binary's embedded trust anchor, so verification is on by default
    with no flag, as on the agent. `--no-verify` (and fold's `--no-sign`/`--eject`,
    which produce no trusted artifact) are the explicit, announced escapes, exactly
    as `--no-verify` already is for per-layer verification (#55, discharged) — an
    explicit opt-out is not opt-in.

    The register consequences, re-derived per proposition rather than by marking
    the shared row discharged wholesale:
    - **#60 discharges** (`Yes`): "no lockfile verification exists" is resolved and
      verification is now mandatory. Its `T9` aspect had no other live ground, so
      **`T9` derives `ENFORCED E1`** on `cmd/strata/run_setverify_test.go` (the set
      signature gates the mount; an unsigned set and a layer appended after signing
      are both refused). `ENFORCED E1` 11 → 12, `REFUTED` 17 → 16.
    - **`T2` stays `REFUTED`**, but on an honest ground: discharging the
      "no verification" counterexample would have flipped it to enforced, so the
      *recorded weakness* the T2 row already named — verification admits any holder
      of the configured key, with no signer-identity policy — is promoted to its
      own counterexample (#225). Overclaiming avoided by splitting, not by leaving
      the row falsely live on a resolved ground.
    - **`T8`/#101's freshness half** was re-keyed to #224 and the row narrowed to
      `T8` only; mix-and-match (`T9`) is no longer part of it.

    (2026-09-15, #101 #225, v0.26.)

95. **The Zenodo deposit becomes a verification bundle (`P2`).** `strata publish`
    deposited the lockfile YAML and nothing else, so a third party got a list of
    digests and a set of registry URIs with nothing to check them against — the
    headline claim (`P2`, independent verifiability) refuted by the artifact the
    project points a DOI at. The deposit now carries, alongside the lockfile
    (whose inline `bundle` is the signature over the whole set), the cosign public
    key (`cosign.pub`, the embedded trust anchor — the public half of the signing
    key, confirmable against KMS by #222) and a `STRATA-VERIFY.md` manifest that
    states the verification procedure and, on the record, that layer *bytes* are
    fetched from a registry with the signed digests as the binding.

    `P2` moves `No` → `Partially`, not to discharged, and the distinction is the
    point: the *attestation* is now independently verifiable from the record alone
    (signature over the set + the key to check it), but the layer **bytes** and the
    per-layer bundles are not deposited — too large for a citation record — so
    byte-level confirmation still needs the registry (position (a) of #99, now
    honestly stated in the deposit rather than promised and not shipped). The set
    signature binds every digest, so the registry can supply bytes but cannot alter
    them undetected. Re-derived at E1: `internal/zenodo/zenodo_bundle_test.go` (the
    key and manifest are deposited; the key is the embedded anchor byte-for-byte).
    No status flips — `P2` stays `REFUTED` on the byte-level gap — so the totals
...
97. **The package installer reads the recorded digest, and refuses a floating
    version (`I6`/`P3`/`X1`/`X2`/`R7`, partially).** `spec.ResolvedPackageEntry`
    has carried a `SHA256` since it was added, documented as the hash of the
    downloaded wheel — and the component that installs packages never read it:
    pip ran `pip install name==version` with no `--hash`, and conda passed a
    recorded version of `latest` (or empty) through as a bare name, so the bytes
    were whatever the channel served at boot. Now pip enforces the digest when it
    is present (`--no-deps --require-hashes … --hash=sha256:<recorded>`) and conda
    refuses a `latest`/empty version outright. The installer functions gained a
    `cmdRunner` seam so the exact invocation is asserted without pip or conda on
    PATH; the `//go:build linux` tests were run under linux (in a container) as
    well as compiled.

    The row moves `No` → `Partially`, not discharged: a pip entry with an *empty*
    recorded digest still installs unverified bytes, so full byte-reproducibility
    waits on the resolver populating every digest, and conda has no per-package
    hash flag. Refusing empty-digest pip entries outright would be stricter but
    would break lockfiles the resolver did not fully hash; that strictness is a
    resolver-side decision, and the operator-facing disclosure of the same gap is
    #139. Both `No` and `Partially` are live, so no status flips and the totals
    hold; the movement is in the row.

    (2026-09-16, #98, v0.27.)

98. **The unattested-packages gap is disclosed where an operator can act
    (`T7`, #139).** Strata's trust story is that what runs was signed and logged,
    but `packages:` entries are installed at boot from PyPI/conda-forge/CRAN with
    no bundle and no Rekor entry — and *nothing said so*. The agent now emits the
    disclosure to the boot log before installing (`unattestedPackagesWarning`,
    behind a `Config.Warnings` writer, `os.Stderr` in `cmd/strata-agent`), and
    `strata verify` prints it (`packagesUnattestedNote`), so an environment whose
    layers all verify no longer silently contains unattested code without saying
    so. #139 closes.

    The `strata run` route was deliberately left silent. `strata run` does not
    install these entries, its silence about attestation is pinned by
    `run_packages_warning_test.go` (inherited, not on the guard's exemption
    allowlist), and adding the clause there would both fail that test and reword a
    counterexample in the change that discharges it — narrowing-until-satisfied.
    So the `T7`/#48 row stays `No` on the run route (its literal subject; the
    wording repair remains #140), even though the substantive disclosure now
    exists at the agent and verify. No derived status flips.

    (2026-09-16, #139, v0.27.)

99. **The whole catalog is pinned and rebuilt from pinned sources, both arches
    (`B2`/`B3`, #229).** Every one of the 32 recipes now pins its source(s) by
    SHA-256, and all 31 buildable ones were rebuilt, signed and pushed for
    **x86_64 and arm64** — 62 layers — from those pinned sources on real EC2
    instances. The `RecipeSource` schema gained a per-arch field so an
    arch-specific binary (nodejs, julia, miniforge, quarto, CUDA) pins one digest
    per arch under a shared staged name; rust's `rustup \| sh` bootstrap was
    replaced by the pinned standalone toolchain; gcc keeps
    `contrib/download_prerequisites`, whose GMP/MPFR/MPC are verified against the
    pinned gcc source's own SHA-512. `knownUnpinnedRecipes` is empty; the guard
    inverted from "these are unpinned" to "every fetching recipe pins."

    Getting there surfaced three build-infra bugs the batch had to fix first, none
    of them recipe defects: the registry index was read-capped at 10 MiB while it
    had grown past that (silently truncating resolution, so `build_requires`
    resolved as "not found"), and `PushLayer` upserted the shared index with a
    plain read-modify-write that concurrent builds raced and lost entries — both
    #233, fixed by raising the cap and switching the upsert to conditional-write
    optimistic concurrency. And `/tmp` on AL2023 is tmpfs (RAM-backed), which the
    CUDA toolkit overflowed regardless of EBS size — fixed by pointing the build's
    `TMPDIR` at the disk-backed root volume.

    `B2`/`B3` do **not** discharge to enforced: #229 pins the *sources*, but the
    build environment — `dnf` "Development Tools" and the AMI's system packages —
    is still installed unpinned from the moving AL2023 repos, so a build is
    reproducible only up to the base image (#234). The `#68` counterexample
    (unpinned recipe sources) is resolved; the build-environment ground keeps
    `B2`/`B3` `REFUTED`. Builds landed in the test registry
    (`s3://strata-registry-test`); promotion to the production layout is the
    deferred canonical-S3 reorg.

    (2026-09-16, #229 #233 #234, v0.27.)
