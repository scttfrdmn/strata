...
verified against primary sources. Of the 34 propositions, 21 are `REFUTED`, 6
are `ENFORCED E1`, 1 is `ENFORCED exhaustive/implementation`, 1 is
`ASSERTED (E0)`, 3 are `UNPOPULATED` and 2 are
...
| R2, R7 | `EnvironmentID` depends on YAML slice order when two layers share a `MountOrder` | H1 | #95 | Partially — the resolver half is fixed: `stage6TopoSort` now tie-breaks the ready set by `lessByLayerContent` (name, version, ID) rather than `sort.Ints(queue)` (input index), so a resolved lockfile's `MountOrder` — and the `EnvironmentID` derived from it — no longer depends on `software:` order. **Re-derived 2026-09-13** at E1 (chosen/implementation): `internal/resolver/mountorder_permutation_test.go`, red under the old `sort.Ints(queue)` and green now. Two things keep the row live, deliberately. (a) The `MountOrder`-tie half survives for **hand-built** lockfiles: the resolver now assigns distinct content-total mount orders so a *resolved* lockfile has no ties, but `computeEnvironmentID` still falls back to slice order when two layers share a `MountOrder` (`spec/lockfile_hash.go:104-110`, a stable sort), so a hand-authored or malformed lockfile with tied orders is still slice-order-dependent — closing that needs the three coupled sort sites (`lockfile_hash.go`, `mount_linux.go`, `oci.go`) to agree on a content tie-break, or the judgement that the resolver guarantee makes it moot. (b) `ProfileSHA256` hashes the profile, so permuting `software:` still changes *that* field, and R2 forbids changing **any** field; narrowing R2 to elide the input-recording fields (as R1 elides `resolved_at`) is a governance call, not a code change. The `Packages`-order clause **was removed** from this row's counterexample (2026-09-13) as a misclassification: package order is content — the installer runs one command per entry in order, so a later entry can shadow an earlier one (`spec/lockfile_hash.go:72-76`, `TestEnvironmentID_PackageOrderIsContent`) — so distinguishing package orders is correct, not a spurious distinction. `Partially` counts as live |
