...
verified against primary sources. Of the 34 propositions, 16 are `REFUTED`, 12
are `ENFORCED E1`, 1 is `ENFORCED exhaustive/implementation`, 1 is
`ASSERTED (E0)`, 2 are `UNPOPULATED` and 2 are
...
| B2, B3 | The build environment itself — `dnf` "Development Tools" and AMI system packages — is consumed by every build but not pinned or named in the recipe | A3 | #234 | No — #229 pinned every recipe *source*, but the compiler/toolchain each build compiles against is installed from the moving AL2023 repos at build time, so the same recipe + pinned sources can still pick up a different gcc/glibc/-devel a month apart. Input completeness (B2) and reproducibility (B3) hold for sources, not for the build environment. Pinning the AMI and/or building against the Strata core layers instead of `dnf` is #234. |
