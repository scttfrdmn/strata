...
verified against primary sources. Of the 34 propositions, 17 are `REFUTED`, 11
...
| **T9** | *(new)* *Set integrity.* The layer set is verified as a set, not layer by layer: a verifier refuses a combination of individually valid layers that was never attested together. | SOUND | none | REFUTED (2 of 2 live) | Added 2026-08-21 (§7). This is TUF's mix-and-match attack. The lockfile is the artifact that names the set, and it is now signed as a whole (`trust.SignLockFile`, v0.26) and verified at `strata verify`, `publish` and the agent, so a set of individually-signed layers that no maintainer attested together has no valid lockfile signature and is refused where verification runs. The row stays live because verification is opt-in: an *unsigned* composed set is still accepted (making it mandatory is the open #101 half), so under A1 an attacker composes a set and simply leaves it unsigned. See the #101 row. |
...
| T8, T9 | No freshness bound and no set-level attestation: rollback, freeze and mix-and-match all succeed | A6, A1 | #101 | Partially — **set-attestation lands (v0.26)**: the lockfile is signed as a whole (`trust.SignLockFile`, #212) and verified at `strata verify`, `publish` and the agent (#216/#217/#218), so **mix-and-match (T9) of a *signed* set is refused** — a combination no maintainer attested together has no valid lockfile signature. Re-derived at E1: `internal/trust/lockfile_test.go`, `internal/agent/agent_lockfile_sig_test.go`. Still live: (a) verification is **opt-in**, so an unsigned composed set is still accepted (mandatory verification is the open decision); (b) **no freshness bound (T8)** — `resolved_at` is now inside the signed payload (so a bound would be tamper-resistant) but nothing refuses a stale, previously-valid signed lockfile, so rollback/freeze still succeed. Both are the remaining #101 work. |
