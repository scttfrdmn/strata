...
| T2, T7, T9 | `strata verify` and `IsSigned()` are presence checks; no lockfile verification exists | A1 | #60 | Partially — **2026-09-14 (v0.26)**. Lockfile verification now exists and is real: `trust.VerifyLockFile` (`internal/trust/lockfile.go`) checks a cosign signature over the whole layer set, and `strata verify` calls it against the public key embedded in the binary (`internal/trust/anchor.go`), so a mix-and-match of individually-valid layers is caught (demonstrated end-to-end: `strata sign` against the KMS key logs to Rekor, `strata verify` confirms with no AWS, and tampering a layer digest fails the check). `trust.SignLockFile` + `strata sign` produce the signature. Both `strata verify` and `strata publish` now verify the set signature — publish refuses a *tampered* lockfile (a non-empty `rekor_entry` with an altered set) before minting a permanent DOI, which the old `IsSigned()` presence check could not catch. **Re-derived** at E1 (chosen/implementation): `internal/trust/lockfile_test.go` (round-trip + tamper/mix-and-match/unsigned), `cmd/strata/sign_test.go`, `cmd/strata/publish_test.go` (rejects unsigned and invalid-signature). Still live, so `Partially`: the EC2 agent does not yet verify the lockfile signature before mounting, an *unsigned* lockfile still passes `verify`'s default path (verification is not yet mandatory), and there is no freshness bound on the signed `resolved_at` — those close as the wiring completes (#101 for the freshness/mandatory half) |
