...
verified against primary sources. Of the 34 propositions, 18 are `REFUTED`, 10
...
| R2 | overlapping formations mount a shared layer twice; permuting them reorders `satisfied_by`/`from_formation` | H1 | #208 | No — **executed 2026-09-14** (review finding #2). Two formations sharing `python`, resolved `[a,b]` vs `[b,a]`: the lockfile has 4 layers (`python` mounted **twice**), an identical `EnvironmentID` (the hashed fields name/version/ID/SHA256 are the same multiset), but `satisfied_by`/`from_formation` on the duplicates swap with `software:` order — fields R2 does not elide, so the full lockfile is not permutation-invariant. `internal/resolver/overlapping_formations_test.go` pins both the double-mount and the order-dependence, red when #208's dedup lands. The v0.25.0 differential instrument (`resolve_determinism_test.go`) used only standalone distinct layers, so `ENFORCED E1` was over that fixture only; `lessByLayerContent`'s final key is the layer ID, which for two instances of one layer is identical, so content cannot break the tie. Fix is dedup-by-content with union provenance (#208), not eliding the fields — that would mask the double-mount |
