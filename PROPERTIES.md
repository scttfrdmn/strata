...
| R2, R7 | `EnvironmentID` depends on YAML slice order when `MountOrder` ties, and on `Packages` order always | H1 | #95 | Partially — the resolver half is fixed: `stage6TopoSort` now tie-breaks the ready set by `lessByLayerContent` (name, version, ID) rather than `sort.Ints(queue)` (input index), so a resolved lockfile's `MountOrder` — and the `EnvironmentID` derived from it — no longer depends on `software:` order. **Re-derived 2026-09-13** at E1 (chosen/implementation): `internal/resolver/mountorder_permutation_test.go`, red under the old `sort.Ints(queue)` and green now. Two parts keep the row live, deliberately. (a) The `Packages`-order clause is a **misclassification**, not a defect: package order is content — the installer runs one command per entry in order so a later entry can shadow an earlier one (`spec/lockfile_hash.go:72-76`, `TestEnvironmentID_PackageOrderIsContent`) — so distinguishing package orders is correct, and the clause is a register correction owed separately, not a fix. (b) `ProfileSHA256` hashes the profile, so permuting `software:` still changes *that* field, and R2 forbids changing **any** field; narrowing R2 to elide the input-recording fields (as R1 elides `resolved_at`) is a governance call, not a code change. `Partially` counts as live |
