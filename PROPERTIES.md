...
`/etc/strata/active.lock.yaml` (`internal/overlay/overlay.go:221-228`) and the
...
`cmd/strata/cache_prune.go:35,56-59,117` reads `Layers[].SHA256` out of it and
...
| **I4** | *Path confinement.* No field of a lockfile or manifest can cause a filesystem operation outside the directory designated for it, for any field value. | SOUND | E1 — `spec/digest_test.go:53` | REFUTED (2 of 4 live) | The mechanism is understood and stated correctly in one place — `spec/digest.go:52-55`: *"`filepath.Join` calls `Clean`, which resolves `..` rather than rejecting it, so joining an unvalidated digest can name a file outside `cacheDir`. Every site that builds a layer cache path must go through here."* Executed confirmation: `filepath.Join("/var/cache/strata/layers", "../../../../etc/cron.d/evil.sqfs")` → `/etc/cron.d/evil.sqfs`. Four sites build a path from an unvalidated lockfile field: `internal/trust/verify.go:92` (`layer.ID`, and its own comment at `:90-91` asserts the opposite — #58); `internal/overlay/mount_linux.go:141` (`layer.ID` into `os.MkdirAll` then `MountSquashfs` — a **write** primitive, untracked); `internal/export/oci.go:60` (`"layer-"+lp.ID`, untracked); `cmd/strata-agent/s3_fetcher.go:73` (`layer.SHA256`, #81). Untracked pair → filed as #97. |
...
| I4 | `internal/overlay/mount_linux.go:141` and `internal/export/oci.go:60` build paths from an unvalidated `layer.ID` | A1 | #97 | No |
...
82. **R4 and R5 discharged — the same instrument that reproduced them is now the barrier.**
    ...
    one predicate, `spec.Capability.Satisfies` (`spec/layer.go:186`), and made the edge follow
...
83. **R7 and X2 quantify over "the assembled environment" and no section defined it,
    ...
    **The exclusion is per field, not per file, and that is the substance.**
    `active.lock.yaml` is not inert: `cmd/strata/cache_prune.go:35,56-59,117` reads
    ...
    `internal/overlay/overlay.go:207-211` (the `PATH`/env-file write) to `:221-228` (the
...
84. **T1 and T5 move `REFUTED` → `ENFORCED E1`; the release refreshes the register
    ...
    `internal/agent/agent.go:299-303` refuses a nil `Verifier`/`BundleFetcher` unless
...
87. **R1 and R2 move to `ENFORCED E1`; both were awaiting one test, and writing
    ...
    **Writing the test found R1's elided set incomplete.** R1 enumerated exactly
    ...
    (`spec/base_digest.go:12`) for the same reason `EnvironmentID` excludes
