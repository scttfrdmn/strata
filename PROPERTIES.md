...
| T8 | rollback/freeze: a stale but previously-valid, correctly-signed lockfile or layer replays undetected | A6 | #224 | No — set-level attestation (`trust.SignLockFile`, #212) and mandatory verification (#101, v0.26) landed, so mix-and-match is refused (see the #60 row); freshness did not. `resolved_at` is inside the signed payload (so a bound would be tamper-resistant), but nothing refuses a stale, previously-valid signed lockfile, so rollback and freeze still succeed. Split from #101 to #224. |
