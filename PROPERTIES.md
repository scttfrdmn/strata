...
| T6 | Nothing records the trust policy a result was produced under | H1 | #100 | Yes — **2026-09-14**. `LockFile.VerificationPolicy` (`spec/lockfile.go`) records what the resolver checked — `attestation-present` when every layer named a well-formed bundle and Rekor entry, `unsigned-offline` when the offline path accepted an unsigned layer — so a consumer reading only the lockfile can tell what was verified. Set by the resolver (`internal/resolver/stages.go`, `verificationPolicy`) and kept out of `EnvironmentID` on the `inert` route (asserted by `TestEnvironmentIDRoutes_MatchBehaviour`, `TestEnvironmentIDRoutes_CoverEveryField`). **Re-derived** at E1 (chosen/implementation): `internal/resolver/verification_policy_test.go`. It records the resolve-time policy, which per #85 is the strongest a resolve can honestly claim; a lockfile-level *verification* result (verify/run against the log) is #60/#101 territory and does not weaken this |
...
89. **T6 is restated from `ILL-FORMED` to `SOUND` and enforced.** Its draft —
    ...
    `attestation-present` (every layer named a well-formed bundle and Rekor entry)
    or `unsigned-offline` (the offline path accepted an unsigned layer), so a
    reader of the lockfile can tell what was checked without knowing how the
    resolve was invoked. It records the resolve-time policy only — a resolve holds
